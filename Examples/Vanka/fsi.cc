...
namespace Gascoigne
{
  ...
  template<int DIM>
  void EQ<DIM>::Form(VectorIterator b, const FemFunction& U, const TestFunction& N) const
  {
    b[1] += N.m();
    for (int i=0;i<DIM;++i)
      for (int j=0;j<DIM;++j)
  ...
  template<int DIM>
  void EQ<DIM>::Matrix(EntryMatrix& A, const FemFunction& U, const TestFunction& M, const TestFunction& N) const
  {    
    for (int i=0;i<DIM;++i)
      for (int j=0;j<DIM;++j)
	{
