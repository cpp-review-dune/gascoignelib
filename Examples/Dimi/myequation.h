...
namespace Gascoigne
{
  ...
  protected:
   ...
   //mutable double eta,zeta,rho;
    mutable double rho;
    mutable FemFunction *oldu, *extu, *H,*W;
    mutable double h_;
  ...
  class TransportEquation : public virtual Equation, public virtual BoundaryEquation
  {
  protected:
    double tau0;
    double shock0;
...
// Rand
  ...
  void Form(VectorIterator b, const FemFunction& U, const TestFunction& N, int col) const
  {
    if (col == 3) // einstroemung
      {
	b[0] += gamma * (U[0].m()-0.5) * N.m();
	b[1] += gamma * (U[1].m()-1) * N.m();
      }
  }
...
/*
// Rand
  void pointboundary(double h, const FemFunction& U, const Vertex2d& v) const
  {
    gamma = gamma0 / h; 
  ...
  void Form(VectorIterator b, const FemFunction& U, const TestFunction& N, int col) const
  {
    if (col == 3) // einstroemung
      {
	b[0] += gamma * (U[1].m()-0.5) * N.m();
	b[1] += gamma * (U[2].m()-1) * N.m();
      }
...
class MyEq : public virtual Equation, public virtual BoundaryEquation
  {
  protected:
    double tau0;
    double shock0;
