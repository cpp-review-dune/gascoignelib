/*----------------------------   ale.h     ---------------------------*/
/*      $Id: ale-dual.h,v 1.2 2010/09/02 09:14:44 richter Exp $                 */
#ifndef __ale_dual_H
#define __ale_dual_H
/*----------------------------   ale.h     ---------------------------*/
...
{
  template<int DIM>
    class AleDual : public virtual LpsEquation
    {
    protected:
      /////////// FSI
      mutable double               __J;
	...
	__U = &q["U"];
      ...
      std::string GetName()  const { return "FSI-Ale-Dual";}
      
      int         GetNcomp() const { return 1+DIM+DIM; }

      void compute_transformation(const FemFunction& U) const;
