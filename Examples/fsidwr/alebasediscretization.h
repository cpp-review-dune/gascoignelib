...
namespace Gascoigne
{
  ...
  public:
    ...
    HASHSET<int>& GetSolidCells()     { return  __solid_cells; }
    HASHSET<int>& GetInterfaceNodes() { return  __interface_nodes; }
    HASHMAP<int,int>& GetFluidG2L()   { return  __fluid_g2l; }
    HASHMAP<int,int>& GetSolidG2L()   { return  __solid_g2l; }
    std::vector<int>& GetFluidL2G()   { return  __fluid_l2g; }
    std::vector<int>& GetSolidL2G()   { return  __solid_l2g; }

    const HASHMAP<int, std::vector<int> >& GetSolidInterfaceCells() const { return  __solid_interface_cells; }
    const HASHMAP<int, std::vector<int> >& GetFluidInterfaceCells() const { return  __fluid_interface_cells; }
    const HASHSET<int>& GetFluidCells()     const { return  __fluid_cells; }
    const HASHSET<int>& GetSolidCells()     const { return  __solid_cells; }
    const HASHSET<int>& GetInterfaceNodes() const { return  __interface_nodes; }
    const HASHMAP<int,int>& GetFluidG2L()   const { return  __fluid_g2l; }
    const HASHMAP<int,int>& GetSolidG2L()   const { return  __solid_g2l; }
    const std::vector<int>& GetFluidL2G()   const { return  __fluid_l2g; }
    const std::vector<int>& GetSolidL2G()   const { return  __solid_l2g; }
    ...
    void DeleteTestFunctionsVector(const LocalVector& F, const std::vector<int>& indices, int comp) const
    {
      LocalVector& __F = const_cast<LocalVector&> (F);
      for (int i=0;i<indices.size();++i) __F(indices[i],comp) = 0.0;
    }

    void DeleteTestFunctionMatrix(EntryMatrix& E, int index, int comp) const
    { for (int j=0;j<E.Mdof();++j) E(index,j,comp,comp) = 0.0; }
    void DeleteAnsatzFunctionMatrix(EntryMatrix& E, int index, int comp) const
    { for (int j=0;j<E.Mdof();++j) E(j,index,comp,comp) = 0.0; }

    void DeleteTestFunctionsMatrix(EntryMatrix& E, const std::vector<int>& indices, int comp) const
    {
      for (int i=0;i<indices.size();++i)
	DeleteTestFunctionMatrix(E,indices[i], comp);
    }
    void DeleteAnsatzFunctionsMatrix(EntryMatrix& E, const std::vector<int>& indices, int comp) const
    {
      for (int i=0;i<indices.size();++i)
    ...
    class AleDiscretization : public AleBaseDiscretization, virtual public BASE
  {
	...
	for (int i=0;i<__DEL_S.size();++i)
      ...
      std::cerr << "Warum hier DeleteAnsatz???? " << std::endl;
      
      if (FluidInterfaceCell(iq))    // In Primal, delete Test-functions
	for (int i=0;i<__DEL_F.size();++i)
	  DeleteAnsatzFunctionsMatrix(E,FluidInterfaceNodes(iq),__DEL_F[i]);
