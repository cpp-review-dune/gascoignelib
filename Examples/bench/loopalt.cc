...
void Loop::run(const std::string& problemlabel)
{
  ...
  VectorInterface u("u"), h("h"), f("f"), oldu("oldu"),hl("hl"), hh("hh"), extu("extu"), div("div"),oldh("oldh"),oldoldu("oldoldu"); 
...
GetMultiLevelSolver()->ReInitVector(oldh);
  ...
  // 	{
  // 	  U(n,0) = WWindX(1000.0*M->vertex2d(n).x(),1000.0*M->vertex2d(n).y(),0.1*T);
  // 	  U(n,1) = WWindY(1000.0*M->vertex2d(n).x(),1000.0*M->vertex2d(n).y(),0.1*T);
  // 	}
