...
UL.zero();
  ...
  nvector<double> maxi(2),mini(2),meanmin(2),meanmax(2),Pminus(2), Pplus(2), Qminus(2), Qplus(2);
  ...
  for (int row = 0; row<ST->n(); ++row)
      ...
      for (int pos = ST->start(row); pos<ST->stop(row); ++pos)
	{
	  ...
	  meanmin[0] += min(0,H(col,0));  meanmin[1]+=min( 0,H(col,1));
	  meanmax[0] += max(0,H(col,0));  meanmax[1]+=max( 0,H(col,1));
      ...
      // meanmin[0]=meanmin[0]/it; meanmin[1]=meanmin[1]/it;
       
      //meanmax[0]=meanmax[0]/it; meanmax[1]=meanmax[1]/it;

      
      
      // mini[0]=0.9*mini[0];mini[1]=0.9*mini[1];
      // maxi[0]=0.9*maxi[0];maxi[1]=0.9*maxi[1];
      
      //  mini[0]=max(0,mini[0]); mini[1]=max(0,mini[1]);
      //maxi[0]=max(1,maxi[0]); maxi[1]=max(1,maxi[1]);
...
void Loop::run(const std::string& problemlabel)
{
  ...
  VectorInterface u("u"), h("h"), hl("hl"), hh("hh"), f("f"), oldu("oldu"),  extu("extu"), div("div"), 
  ...
  for (_iter=1; _iter<=_niter; _iter++)
      ...
      for (int sub=0;sub<NSUB;++sub)
	{
	  ...
	  SolveDIV(div,u,f);
	  SolveTransport( h,hl,hh,div, u, f);
	    GlobalVector& H = GetMultiLevelSolver()->GetSolver()->GetGV(h);
      for (int i=0;i<H.n();++i)
	{
      ...
      //   if (_iter%196==0)
      //GetMultiLevelSolver()->Equ(h,1.0,hl);
  ...
  for (int p=0;p<M->npatches();++p)
    {
      ...
      for (int c=0;c<2;++c)
    {
      if(fabs( H(iop[1],c)-H(iop[0],c))<1.e-3 ||fabs( H(iop[1],c)-H(iop[2],c))<1.e-3)
      H(iop[1],c) = 0.5*H(iop[0],c)+0.5*H(iop[2],c);
      if(fabs( H(iop[3],c)-H(iop[0],c))<1.e-3 ||fabs( H(iop[3],c)-H(iop[6],c))<1.e-3)
      H(iop[3],c) = 0.5*H(iop[0],c)+0.5*H(iop[6],c);
      if( fabs(H(iop[5],c)-H(iop[2],c))<1.e-3 ||fabs( H(iop[5],c)-H(iop[8],c))<1.e-3)
      H(iop[5],c) = 0.5*H(iop[2],c)+0.5*H(iop[8],c);
      if(fabs( H(iop[7],c)-H(iop[6],c))<1.e-3 ||fabs( H(iop[7],c)-H(iop[8],c))<1.e-3)
      H(iop[7],c) = 0.5*H(iop[6],c)+0.5*H(iop[8],c);
      if( fabs(H(iop[4],c)-H(iop[0],c))<1.e-3 ||fabs( H(iop[4],c)-H(iop[2],c))<1.e-3 || fabs( H(iop[4],c)-H(iop[6],c))<1.6e-3 ||fabs( H(iop[4],c)-H(iop[8],c))<1.e-3)  
      H(iop[4],c) = 0.25*H(iop[0],c)+0.25*H(iop[2],c)+0.25*H(iop[6],c)+0.25*H(iop[8],c);
    }
    }
      */    
