...
#include "multiplex.xx"
 ...
 template<int DIM>
  void FSI<DIM>::point_cell(int material) const 
{
 ...
 template<int DIM>
    ...
    if (domain>0)
      {
		...
		if(mat_law=="STVK")
		{
				E = 0.5*(F_PRES.inverse().transpose()*F.transpose()*F*F_PRES.inverse()-MATRIX::Identity());
				MATRIX E_old = 0.5*(F_PRES.inverse().transpose()*F_old.transpose()*F_old*F_PRES.inverse()-MATRIX::Identity());
				
				MATRIX SIGMAs_el=(2.0 * mu_s * E + lambda_s * E.trace() * MATRIX::Identity());
				MATRIX SIGMAs_el_old=(2.0 * mu_s * E_old + lambda_s * E_old.trace() * MATRIX::Identity());
		  		SIGMAs = J_PRES*F_PRES.inverse()*SIGMAs_el*F_PRES.inverse().transpose();
		  		SIGMAs_old = J_PRES*F_PRES.inverse()*SIGMAs_el_old*F_PRES.inverse().transpose();
		  }
		  else if(mat_law=="artery")
		  {
			C =F_PRES.inverse().transpose()*F.transpose()*F*F_PRES.inverse();
			MATRIX C_old =F_PRES.inverse().transpose()*F_old.transpose()*F_old*F_PRES.inverse();
			
			MATRIX SIGMAs_el=mu_s*pow(J,-2.0/3.0)*(MATRIX::Identity()-1.0/3.0*C.trace()*C.inverse())+0.5*kapa_s*(pow(J,2.0)-1.0)*C.inverse();
			MATRIX SIGMAs_el_old=mu_s*pow(J_old,-2.0/3.0)*(MATRIX::Identity()-1.0/3.0*C_old.trace()*C_old.inverse())+0.5*kapa_s*(pow(J_old,2.0)-1.0)*C_old.inverse();
		  	SIGMAs = J_PRES*F_PRES.inverse()*SIGMAs_el*F_PRES.inverse().transpose();
		  	SIGMAs_old = J_PRES*F_PRES.inverse()*SIGMAs_el_old*F_PRES.inverse().transpose();
		  }
		  else abort();
  ...
  if(domain>0)
	...
	for (int j=0;j<DIM;++j) // Fj Sigma \nabla phi
	  ...
	  A(j+1,j+1) += __THETA * (SIGMA_dF[j].transpose()*phi)(0,0) * __THETA * __DT;
 ...
 void FSI<DIM>::point_M(int j, const FemFunction& U, const TestFunction& M) const
  {
	...
	*/
			  if(mat_law=="STVK")
			{
					...
					for (int j=0;j<DIM;++j)
						{
							MATRIX Ej=  0.5 * (F_PRES.inverse().transpose()*psi*(F.block(j,0,1,DIM))*F_PRES.inverse()+
									   F_PRES.inverse().transpose()*F.block(j,0,1,DIM).transpose() * psi.transpose()*F_PRES.inverse());
							MATRIX SIGMA_dU_el = (2.0*mu_s*Ej + lambda_s * Ej.trace()*MATRIX::Identity());
							SIGMA_dU[j] = F*J_PRES*F_PRES.inverse()*SIGMA_dU_el*F_PRES.inverse().transpose();
						}
			}
			  else if(mat_law=="artery")
			  {
				...
				for (int j=0;j<DIM;++j)
					{				 				 
						MATRIX C_dU=  F_PRES.inverse().transpose()*(psi*F.block(j,0,1,DIM)+F.block(j,0,1,DIM).transpose() * psi.transpose())*F_PRES.inverse();
						MATRIX C_dU_inverse= - C.inverse()*C_dU*C.inverse();
						double J_dU=J*(F.inverse().block(0,j,DIM,1)*psi.transpose()).trace();
					
						MATRIX SIGMA_dU_el = mu_s*(-2.0/3.0)*pow(J,-5.0/3.0)*J_dU*(MATRIX::Identity()-1.0/3.0*C.trace()*C.inverse());
						SIGMA_dU_el +=mu_s*pow(J,-2.0/3.0)*(-1.0/3.0*C_dU.trace()*C.inverse());
						SIGMA_dU_el +=mu_s*pow(J,-2.0/3.0)*(-1.0/3.0*C.trace()*C_dU_inverse);
