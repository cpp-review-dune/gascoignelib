...
void BoundaryFSI<DIM>::Form(VectorIterator b, const FemFunction &U_Dummy,
  ...
  g = -__rho_f * __nu_f * J * F.inverse().transpose() * NV.transpose() *
      F.inverse().transpose() * __n;
  g_OLD = -__rho_f * __nu_f * J_OLD * F_OLD.inverse().transpose() *
          NV_OLD.transpose() * F_OLD.inverse().transpose() * __n;
  ...
  if (col == 4) {
    for (int i = 0; i < DIM; ++i) {
      b[i + 1] += __THETA * g(i) * N.m() + (1.0 - __THETA) * g_OLD(i) * N.m() +
                  __n[i] * fact_time * (p_2)*N.m();
...
void BoundaryFSI<DIM>::Matrix(EntryMatrix &A, const FemFunction &U_Dummy,
  ...
  for (int j = 0; j < DIM; ++j) {
    ...
    g = -__rho_f * __nu_f * J * F.inverse().transpose() * NPHI.transpose() *
        F.inverse().transpose() * __n;
