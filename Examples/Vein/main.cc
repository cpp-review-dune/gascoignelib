      ...
      double J(const Gascoigne::FemFunction &U, const Gascoigne::Vertex2d &v, 
          const Gascoigne::Vertex2d &n, int color) const
          {
				 ...
				 //Initialisieren der Vektoren und Matrizen
					...
					SIGMA_F = -__J*P*F.inverse().transpose()+ __J *  __rho_f * __nu_f *
						(
						 NV*F.inverse() 
						 + F.inverse().transpose()*NV.transpose()
						 )  * F.inverse().transpose();
...
int main(int argc, char** argv)
{
  ...
  vector<Vertex2d> v2d;v2d.push_back(Vertex2d(0.6,0.2));
  vector<int> cx,cy; cx.push_back(0);cy.push_back(1);
  ...
  vector<int> cx2,cy2; cx2.push_back(1);cy2.push_back(2);
