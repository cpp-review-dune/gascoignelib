...
namespace Gascoigne
{
  ...
  template<int DIM>
  void FSIVankaSolver<DIM>::smooth(int niter, VectorInterface& x, const VectorInterface& y, VectorInterface& h) const 
  {			
    ...
    if (StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi_av"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_gs")
      {
					...
					//	dynamic_cast<const SparseBlockMatrix<FMatrixBlock<ncomp> >*> (StdSolver::GetMatrix());
...
//								Eigen::VectorXd X = Vanka_MV->GetMatrix(p).inverse()*H;
							...
							//// Residual
								if(vanka_type=="vanka_gs") 
								{
										...
										cout<<"uncomment the following line and declaration of A"<<endl; abort();
...
/*											 for (auto row : Vanka_MV->Get_Vector_Columns(p))
						...
						for (int i=0;i<gvX.n();++i)
							{
								...
								{	
									if(smooth_weight[i]==0) {cout<<"smooth_weight[i]==0"<<endl; abort();}
									else gvX(i,ii) +=smooth_zwisch(i,ii)/smooth_weight[i];
								}
							}
					}
			}
  ...
  template<int DIM>
  void FSIVankaSolver<DIM>::invert_local_matrices() const
	{    
		//const int ncomp=4;
		//const int ncomp=StdSolver::GetProblemDescriptor()->GetEquation()->GetNcomp();
		//StdSolver::GetProblemDescriptor()->GetEquation()->GetNcomp();  
...
/* -------------------------------------------------------*/
  ...
  template<int DIM>
  void FSIVankaSolver<DIM>::ComputeIlu() const
{
		if ( (StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi_av"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_gs" )&& StdSolver::_directsolver==false)
    { 
...
/* -------------------------------------------------------*/
  template<int DIM>
  void FSIVankaSolver<DIM>::RegisterMatrix()
{
	StdSolver::RegisterMatrix();
}
  template<int DIM>
  void FSIVankaSolver<DIM>::SetILUPointer(int ncomp)
  {
  		  ...
  		  if ( (StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_jacobi_av"||StdSolver::GetSolverData().GetLinearSmooth()=="vanka_gs" )&& StdSolver::_directsolver==false)
		  {
		  	const GascoigneMesh* M = dynamic_cast<const GascoigneMesh*> (StdSolver::GetMesh());
				...
				if(Vanka_MV==NULL)
				{
					...
					else cout<<" Vanka_Matrix_Vector not written for "<<ncomp;
				}	
...
/* -------------------------------------------------------*/
  template<int DIM>
  void FSIVankaSolver<DIM>::ReInitMatrix()
  {
	...
	if ((StdSolver::GetSolverData().GetLinearSmooth()!="vanka_jacobi"&&StdSolver::GetSolverData().GetLinearSmooth()!="vanka_jacobi_av"&& StdSolver::GetSolverData().GetLinearSmooth()!="vanka_gs")|| StdSolver::_directsolver==true )	
