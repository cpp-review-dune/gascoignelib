  ...
  const nvector<int> IndicesSmoothingCell(int p) const
  {
    ...
    /*
        const GascoigneMesh* M = dynamic_cast<const GascoigneMesh*> (GetMesh());
                                      nvector<int> iop;
                                            nvector<int> cellsofpatch =
       M->GetPatchIndexHandler().GetPatch2Cell(p/2);
       ...
       itttt=patch_part_1.begin();itttt!=patch_part_1.end();itttt++)
                                            {
                                                    nvector<int>
       indcell=M->IndicesOfCell(cellsofpatch[*itttt]); vector<int>::iterator it;
                                                    for (it=indcell.begin();
       it!=indcell.end(); ++it) indices_half_patch_1.insert(*it);
                                            }
                                            for(set<int>::iterator
       itttt=patch_part_2.begin();itttt!=patch_part_2.end();itttt++)
                                            {
                                                    nvector<int>
       indcell=M->IndicesOfCell(cellsofpatch[*itttt]); vector<int>::iterator it;
                                                    for (it=indcell.begin();
       it!=indcell.end(); ++it) indices_half_patch_2.insert(*it);
                                            }
                                    ...
                                    if(p%2==0)
                                     {
                                             set<int>::iterator it;
                                             for
       (it=indices_half_patch_1.begin(); it!=indices_half_patch_1.end(); ++it)
                                                    iop.push_back(*it);
                                     }
                                    else
                                     {
                                             set<int>::iterator it;
                                             for
       (it=indices_half_patch_2.begin(); it!=indices_half_patch_2.end(); ++it)
                                                    iop.push_back(*it);
                                     }
                                     return iop;
                                    */
    ...
    /*
                                             const GascoigneMesh* M =
       dynamic_cast<const GascoigneMesh*> (GetMesh()); nvector<int> iop;
                                                    nvector<int> cellsofpatch =
       M->GetPatchIndexHandler().GetQ4Patch2Cell(p/8); set<int>
       indices_half_patch;

                                                    vector<int> pp;
       pp.push_back(0);pp.push_back(1);pp.push_back(2);pp.push_back(3);pp.push_back(4);pp.push_back(5);pp.push_back(6);pp.push_back(7);


                                                    for(int ii=0;ii<8;ii++)
                                                    {
                                                            nvector<int>
       indcell=M->IndicesOfCell(cellsofpatch[pp[p%8]*8+ii]);
                                                            vector<int>::iterator
       it; for (it=indcell.begin(); it!=indcell.end(); ++it)
                                                                    indices_half_patch.insert(*it);
                                                    }

                                                     set<int>::iterator it;
                                                     for
       (it=indices_half_patch.begin(); it!=indices_half_patch.end(); ++it)
                                                            iop.push_back(*it);
                                            return iop;
                            */
  }
