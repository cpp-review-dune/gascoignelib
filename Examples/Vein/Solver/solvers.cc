...
void FSISolver<DIM>::DeleteSolidPressure(VectorInterface &gf) const {
  ...
  */
  ...
  if (GetProblemDescriptor()->GetName() != "fluid_stat" ||
      GetProblemDescriptor()->GetName() != "solid_euler") {
...
GetAleDiscretization()->GetFluidL2G().end())
            {
                  if (GetAleDiscretization()->GetInterfaceNodes().find(i)!=
GetAleDiscretization()->GetInterfaceNodes().end()) domain=0; else domain=-1;
            }
          else
            {
                  if (find (GetAleDiscretization()->GetSolidL2G().begin(),
GetAleDiscretization()->GetSolidL2G().end(), i) !=
GetAleDiscretization()->GetSolidL2G().end())
                    {
                  if (GetAleDiscretization()->GetInterfaceNodes().find(i)!=
GetAleDiscretization()->GetInterfaceNodes().end()) domain=0; else domain=1;
                    }
                  else {cout<<"error writing mesh. Node neither fluid nor
solid"<<endl; abort();}
            }
