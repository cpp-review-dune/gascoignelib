...
namespace Gascoigne
{
  ...
  template<int DIM>
  void Boundary_Fluid_Stat<DIM>::Form(VectorIterator b, const FemFunction& U, const TestFunction& N, int col) const
  {
		...
		for (int i=0;i<DIM;++i)
			{
			 ...
			 if((U[1].m()*__n[0]+U[2].m()*__n[1]+U[3].m()*__n[2])<0)
			 		b[i+1] -= 0.5*(U[1].m()*__n[0]+U[2].m()*__n[1]+U[3].m()*__n[2])*U[1+i].m()*N.m();
			}
	}
  ...
  template<int DIM>
   void Boundary_Fluid_Stat<DIM>::Matrix(EntryMatrix& A, const FemFunction& U, const TestFunction& M, const TestFunction& N, int col) const
   {
			...
			for(int i=0;i<DIM;++i)
         	{
					...
					if((U[1].m()*__n[0]+U[2].m()*__n[1]+U[3].m()*__n[2])<0)
					{
						A(1+i,j+1)  -= 0.5*(U[1].m()*__n[0]+U[2].m()*__n[1]+U[3].m()*__n[2])*PHI[i]*N.m();
					 	A(1+i,j+1) 	-= 0.5*(PHI[0]*__n[0]+PHI[1]*__n[1]+PHI[2]*__n[2])*U[1+i].m()*N.m();      	 
					}
			}
  ...
  template<int DIM>
   void Boundary_Fluid_Stat<DIM>::pointboundary(double h, const FemFunction& U, const Vertex<DIM>& v, const Vertex<DIM>& n) const
  {
