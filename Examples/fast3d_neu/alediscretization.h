      ...
      for (int i = 0; i < q.size(); i++) {
        ...
        if (__ADJOINT) {
          if (FluidInterfaceCell(ip))
            for (int i = 0; i < __DEL_F.size(); ++i)
              DeleteTestFunctionsVector(
                __U, FluidInterfaceNodes(ip), __DEL_F[i]);
          if (SolidInterfaceCell(ip))
            for (int i = 0; i < __DEL_S.size(); ++i)
              DeleteTestFunctionsVector(
                __U, SolidInterfaceNodes(ip), __DEL_S[i]);
        }
        ...
        if (!__ADJOINT) {
          if (FluidInterfaceCell(ip))
            for (int i = 0; i < __DEL_F.size(); ++i)
              DeleteTestFunctionsVector(
                __F, FluidInterfaceNodes(ip), __DEL_F[i]);
          if (SolidInterfaceCell(ip))
            for (int i = 0; i < __DEL_S.size(); ++i)
              DeleteTestFunctionsVector(
                __F, SolidInterfaceNodes(ip), __DEL_S[i]);
        }
