...
class FSI_CI : public ComponentInformationBase
{
 public:
  ...
  void      GetVectorIndices(int i, fixarray<3,int>& fa_vectorindices) const
  {
	...
	fa_vectorindices[2]=-1;
	if (DIM==3) fa_vectorindices[3]=DIM+3;
      }
  }
...
//             *__EINSTROMGESCHWINDIGKEIT* v.y() * (1.61-v.y()) *4/1.61/1.61;
    ...
    if (color==7)
      {
				Vertex3d center; center[0]=3.78223304703; center[1]=-3.882233047; center[2]=1.0928932;
				Vertex3d normal;  normal[0]=  0.5000  ; normal[1]= -0.4998 ; normal[2]=   0.7072;
				double diam=0.3;
				b[1] = -normal[0]*fact_time * ( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
			 	b[2] = -normal[1] *fact_time *( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
				b[3] = -normal[2]*fact_time * ( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
      }
    */
...
class MyDD3d : public DirichletData
{
 ...
 public:
  ...
  void operator()(DoubleVector& b, const Vertex3d& v, int color) const
  {
    ...
    if (color==0)
      b[1] += v.y()*(0.4-v.y())/0.2/0.2 * (0.4-v.z())*(0.4+v.z())/0.4/0.4 * vmean * sc * 9.0/8.0;
    ...
    if (color==7)
      {
				...
				Vertex3d normal;  normal[0]=  0.5000  ; normal[1]= -0.4998 ; normal[2]=   0.7072;
				double diam=0.3;
				b[1] += -normal[0]*1.0e-1*fact_time * ( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
			  b[2] += -normal[1]*1.0e-1 *fact_time *( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
				b[3] += -normal[2]*1.0e-1*fact_time * ( (x-center[0])*(x-center[0])+(y-center[1])*(y-center[1])+(z-center[2])*(z-center[2])-diam*diam)/(-diam*diam);
      }
  }
};
