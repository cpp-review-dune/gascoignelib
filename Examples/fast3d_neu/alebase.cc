...
void AleBase::compute_transformation(const Vertex3d &v, const FemFunction &U,
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 2; ++j)
      __nablaV_Fi(i, j) =
          U[i + 1][j + 1] - v.z() * H[0][j + 1] * U[i + 1].z() / __J;
  for (int i = 0; i < 3; ++i)
...
double AleBase::DV_nablaV_Fi(int i, int j, int d, const FemFunction &U,
                             const FemFunction &H,
