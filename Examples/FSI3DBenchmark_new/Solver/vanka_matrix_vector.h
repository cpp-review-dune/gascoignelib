...
public:
  void build_patches(const GascoigneMesh *M) {
    ...
    for (int p = 0; p < M3->npatches(); ++p) {
      if (patchesofsamecol.find(M3->material_Vanka_patch(p)) !=
          patchesofsamecol.end())
    ...
    for (int p = 0; p < M3->npatches(); ++p) {
      ...
      // Vertex3d
      // mid(M3->vertex3d(iop[13]).x(),M3->vertex3d(iop[13]).y(),M3->vertex3d(iop[13]).z());
    ...
    // Problem bei komplexeren Geometrien Patche in X nur in z-Koordinate
    // sortiert Blocken von jeweils n*n patches und sortieren dieser Bloecke in
    // y Richtung Blocken von n patches in jedem Block und sortieren in
    // x-Richtung
    for (auto iterX : X) {
      ...
      // Fasse n Patches(sortiert nach y) in jedem der n Bloecke zusammen und
      // sortiere diese nach x
    ...
    for (auto iter_Box : Z) {
      ...
      if (patchesofsamecol[iter_Box.first] == 1)
        combine_n_patches = 1;
      if (patchesofsamecol[iter_Box.first] == 8)
        combine_n_patches = 2;
      if (patchesofsamecol[iter_Box.first] == 8 * 8)
        combine_n_patches = 4;
      if (patchesofsamecol[iter_Box.first] == 8 * 8 * 8)
        combine_n_patches = 8;
     ...
     for (int p=0;p<patchindices.size();++p)
      {
           ...
           {
                   func_log <<M3->vertex3d(patches)[0]<<"
     "<<M3->vertex3d(patches)[1]<<" "<<M3->vertex3d(patches)[2]<<" ";
           }
           func_log <<endl;
      }
      */
