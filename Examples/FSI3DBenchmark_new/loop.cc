...
Loop<DIM>::run(const std::string& problemlabel)
{
       ...
       for (int i=0;i<2;++i)
        {
                                  ...
                                  const IntVector* refnodes1 =
     GetMultiLevelSolver()->GetSolver()->GetMesh()->VertexOnBoundary(80); const
     IntVector* refnodes2 =
     GetMultiLevelSolver()->GetSolver()->GetMesh()->VertexOnBoundary(81); const
     IntVector* refnodes3 =
     GetMultiLevelSolver()->GetSolver()->GetMesh()->VertexOnBoundary(90);
  ...
  // fsi_main 		Zum Abspeichern von Vektoren. Keine Matrix!
  // Anlegen mit Problem: "fsi_main" fsi_reduced 		Matrix mit
  // (DIM+1)*(DIM+1)						Anlegen mit
  // Problem: "fsi_reduced" meshmotion 		Matrix mit DIM*DIM
  // Anlegen mit Problem: "meshmotion"
  // def_solid		Matrix mit DIM*DIM
  // Anlegen mit Problem: "def_solid"
  ...
  /* for (int i=0;i<_initial_refine;++i)
      {
        ...
        GetMultiLevelSolver()->ReInitVector(f);
                ...
                // set offset first, so nodes that are both periodic and
     dirichlet will become dirichlet
                ...
                //////////////////////////////////////////////////////////////////////////////
                cout<<"================================================="<<endl;
                cout<<"Only Solid Euler to compute prestress on solid
     domain!!"<<endl;
                ...
                if(i==0)
     GetMultiLevelSolver()->GetSolver()->GetGV(def_pres).zero(); else
     GetMultiLevelSolver()->InterpolateSolution(def_pres,def_pres_zw);
  ...
  for (_iter = 1; _iter <= _niter; _iter++) {
    ...
    */
    ...
    // func_log <<__TIME << "\t" << 0.0 << "\t" << juh<<"\t"<<juh3[5]<<"
    // "<<juh3[7]<<"  "<<juh2[8]<<"  "<<juh2[9]<< endl;
  }
  FSI_MLS->SetSolverLabel("fsi_main");
