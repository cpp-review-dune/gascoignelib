...
void FSIMatrix<DIM>::entry(nvector<int>::const_iterator start,
  ...
  if (nfluid == nnodes) {
    ...
    cerr << "FSIMatrix entry: " << nfluid << " " << nsolid << " " << nnodes << endl;
    assert(0);
  }
}
...
void FSIMatrix<DIM>::dirichlet(const nvector<int>& bv, const std::vector<int>& cv) {
  ...
  for (int i= 0; i < bv.size(); ++i) {
    if (fluid_node(bv[i]))
      bf.push_back(Fg2l(bv[i]));
    if (solid_node(bv[i]))
      bs.push_back(Sg2l(bv[i]));
  }
  ...
  for (int i= 0; i < cv.size(); ++i) {
    ...
    if (c <= DIM)
      ...
      cext.push_back(c - DIM - 1);

    if (c <= DIM)
      cs1.push_back(c - 1);
    else
      cs2.push_back(c - 1 - DIM);
  }
...
void FSIMatrix<DIM>::dirichlet(int ii, const std::vector<int>& cv) {
  if (fluid_node(ii)) {
    ...
    for (int i= 0; i < cv.size(); ++i) {
      if (cv[i] <= DIM)
        cv_ns.push_back(cv[i]);
      else
        cv_ext.push_back(cv[i] - DIM - 1);
    }
    for (int i= 0; i < cv_ns.size(); ++i)
      assert(cv_ns[i] <= DIM);
    for (int i= 0; i < cv_ext.size(); ++i)
      assert(cv_ext[i] < DIM);
    for (int i= 0; i < cv_ext.size(); ++i)
      assert(cv_ext[i] >= 0);
  ...
  if (solid_node(ii)) {
    ...
    for (int i= 0; i < cv.size(); ++i) {
      if (cv[i] <= DIM)
        cv1.push_back(cv[i] - 1);
      else
        cv2.push_back(cv[i] - 1 - DIM);
    }

    for (int i= 0; i < cv1.size(); ++i)
      assert((cv1[i] >= 0) && (cv1[i] < DIM));
    for (int i= 0; i < cv2.size(); ++i)
      assert((cv2[i] >= 0) && (cv2[i] < DIM));
...
template <int DIM>
void FSIMatrix<DIM>::Fluid_l2g_add(GlobalVector&       x,
...
template <int DIM>
void FSIMatrix<DIM>::Solid1_l2g_add(GlobalVector&       x,
...
template <int DIM>
void FSIMatrix<DIM>::Solid2_l2g_add(GlobalVector&       x,
                                    const GlobalVector& xs,
                                    double              s) const {
  assert(xs.n() == __solid_l2g->size());
  assert(xs.ncomp() == DIM);
...
template <int DIM>
void FSIMatrix<DIM>::Fluid_NS_l2g_add(GlobalVector&       x,
                                      const GlobalVector& xf,
                                      double              s) const {
  assert(xf.n() == __fluid_l2g->size());
  assert(xf.ncomp() == DIM + 1);
...
template <int DIM>
void FSIMatrix<DIM>::Fluid_EXT_l2g_add(GlobalVector&       x,
                                       const GlobalVector& xf,
                                       double              s) const {
  assert(xf.n() == __fluid_l2g->size());
  assert(xf.ncomp() == DIM);
