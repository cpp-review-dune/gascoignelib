...
// F.inverse().transpose()*FIJ.transpose()) )  * normal; 	    double BOUNDARY_V = rho_f *
// nu_f * N.m() * J * 	      (psi.transpose() *
// (F.inverse().transpose()*F.inverse().transpose())*normal)(0,0); 	    for (int
// i=0;i<DIM;++i)
// 	      {
...
//   NV.transpose()*F.inverse().transpose()*F.inverse().transpose()*normal; BOUNDARY_old =
//   rho_f * nu_f * J_old *
