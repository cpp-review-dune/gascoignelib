...
public:
  ...
  const SparseBlockMatrix<DiagBlock<DIM>>& GetS11() const {
    return __S11;
  }
  const SparseBlockMatrix<FMatrixBlock<DIM>>& GetS12() const {
    return __S12;
  }
  const SparseBlockMatrix<DiagBlock<DIM>>& GetS21() const {
    return __S21;
  }
  const SparseBlockMatrix<FMatrixBlock<DIM>>& GetS22() const {
    return __S22;
  }

  const SparseBlockMatrix<FMatrixBlock<DIM + 1>>& GetF_NS() const {
    return __F_NS;
  }
  const SparseBlockMatrix<DiagBlock<DIM>>& GetF_EXT() const {
    return __F_EXT;
  }
  const SparseBlockMatrix<FMatrixBlockNM<DIM + 1, DIM>>& GetF_ALE() const {
    return __F_ALE;
  }

  const MatrixInterface* GetFluid_NS() const {
    return &__F_NS;
  }
  const MatrixInterface* GetFluid_EXT() const {
    return &__F_EXT;
  }
  const MatrixInterface* GetFluid_ALE() const {
    return &__F_ALE;
  }
  const MatrixInterface* GetSolid11() const {
    return &__S11;
  }
  const MatrixInterface* GetSolid12() const {
    return &__S12;
  }
  const MatrixInterface* GetSolid21() const {
    return &__S21;
  }
  const MatrixInterface* GetSolid22() const {
    return &__S22;
  }
  ...
  virtual const StencilInterface* GetStencil() const {
    assert(0);
  }
