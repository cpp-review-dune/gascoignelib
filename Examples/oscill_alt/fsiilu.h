...
public:
    ...
    void init_mg(int l, int cl, vector<const FSIIlu<DIM>*> ILUH, const MgInterpolatorNested& mgint,
                 const nvector<int>& bn)
    {
        //      assert(bn.size()==0);
        thislevel   = l;
        coarselevel = cl;
        __FSI_H.resize(ILUH.size());
        for (int i = 0; i < ILUH.size(); ++i)
            __FSI_H[i] = ILUH[i];
    ...
    const SparseBlockIlu<DiagBlock<DIM>>& GetSM() const
    {
        return __SM;
    }
    const SparseUmf<FMatrixBlock<DIM>>& GetSES() const
    {
        return __SES;
    }
    ...
    const SparseBlockIlu<FMatrixBlock<DIM + 1>>& GetF_NS() const
    {
        return __F_NS;
    }
    //    const SparseUmf<FMatrixBlock<DIM+1> >& GetF_NS()  const { return __F_NS; }

    const SparseBlockIlu<DiagBlock<DIM>>& GetF_EXT() const
    {
        return __F_EXT;
    }
    ...
    void ConstructStructure(const IntVector& permlfuid, const IntVector& permsolid,
    ...
    void vmult_NS_dirichlet(const MatrixInterface& A, GlobalVector& y, GlobalVector& x,
                            double s) const;
    void vmult_EXT_dirichlet(const MatrixInterface& A, GlobalVector& y, GlobalVector& x,
                             double s) const;

    void Jacobi(const MatrixInterface& A, GlobalVector& x) const;
    ...
    int BiCGSTAB(const MatrixInterface& A, GlobalVector& x, const GlobalVector& b,
                 const IluInterface& M, int& max_iter, double& tol) const;
    int BiCGSTAB(const MatrixInterface& A1, const MatrixInterface& A2, GlobalVector& x,
                 const GlobalVector& b, const IluInterface& M, int& max_iter, double& tol) const;
    int CG(const MatrixInterface& A, GlobalVector& x, const GlobalVector& b, const IluInterface& M,
           int& max_iter, double& tol) const;

    int CGSolid(const MatrixInterface& A, GlobalVector& x, const GlobalVector& b,
                const IluInterface& M, int& max_iter, double& tol) const;
