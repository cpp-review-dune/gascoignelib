...
public:
  ...
  void init_mg(int                         l,
               int                         cl,
               vector<const FSIIlu<DIM>*>  ILUH,
               const MgInterpolatorNested& mgint,
  ...
  void ConstructStructure(const IntVector&       permlfuid,
  ...
  // vmult that does the correct interface handling even tho A has no dirichlet values
  // embedded!
  ...
  int BiCGSTAB(const MatrixInterface& A,
               ...
               int&                   max_iter,
               double&                tol) const;
  int BiCGSTAB(const MatrixInterface& A1,
