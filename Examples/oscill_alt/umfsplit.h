...
public:
  ...
  nvector<double>& GetRaw() {
    return __Ax;
  }
  vector<long>& GetRawColumn() {
    return __Ac;
  }
  vector<long>& GetRawPosition() {
    return __Ap;
  }

  int n() const {
    std::cerr << "UmfSplit::n()" << std::endl;
    abort();
  }
  ...
  void ConstructStructureFluid(const MatrixInterface& A,
                               const vector<int>&     fluid_l2g,
                               const HASHSET<int>&    interface_nodes);
  void ConstructStructureSolid(const MatrixInterface& A,
                               const vector<int>&     solid_l2g,
                               const HASHSET<int>&    interface_nodes);
