  ...
  if (domain < 0) {
    ...
    for (int i= 0; i < DIM; ++i) {
      if (bdf1()) {
        ...
        b[i + 1 + DIM]+= scaling
                         * (1.5 * U[i + 1 + DIM].m() - 2. * (*OLD)[i + 1 + DIM].m()
                            + .5 * (*OLDOLD)[i + 1 + DIM].m())
                         / GetTimeStep() * N.m();
      }
      b[i + 1 + DIM]-= scaling * U[i + 1].m() * N.m();
    }
  }
#else
  if (domain < 0)  // fluid
...
template <int DIM>
void FSI<DIM>::Matrix(EntryMatrix&        A,
                      const FemFunction&  U,
  ...
  if (domain < 0) {
    ...
    for (int j= 0; j < DIM; ++j) {
      ...
      VECTOR Y= -rho_f * J * NV * Fij[j] * dtU / GetTimeStep() * N.m();
      for (int i= 0; i < DIM; ++i)
        A(i + 1, j + 1 + DIM)+= Y(i, 0);
    }
    ...
    for (int j= 0; j < DIM; ++j)  // Fj Sigma \nabla phi
      A(j + 1, j + 1 + DIM)+= (SIGMA_dF[j].transpose() * phi)(0, 0);
...
void FSI<DIM>::point_M(int j, const FemFunction& U, const TestFunction& M) const {
  ...
  if (domain < 0) {
    ...
    DOMAIN_U1= -rho_f * J * NV * F.inverse() / GetTimeStep();
    DOMAIN_U2= -rho_f * NV * F.inverse() * dtU / GetTimeStep();
    ...
    for (int j= 0; j < DIM; ++j) {
      ...
      DOMAIN_V[j]=
        -rho_f * J * (psi.transpose() * F.inverse() * dtU / GetTimeStep())(0, 0);
...
template <int DIM>
void FSI<DIM>::MatrixBlock(EntryMatrix&       A,
                           const FemFunction& U,
...
template <int DIM>
void FSI<DIM>::StabForm(VectorIterator      b,
                        const FemFunction&  U,
...
template <int DIM>
void FSI<DIM>::StabMatrix(EntryMatrix&        A,
                          const FemFunction&  U,
