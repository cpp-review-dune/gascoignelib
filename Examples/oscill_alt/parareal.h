...
struct stepper
{
    static inline constexpr void propagator(size_t n_steps, size_t implicit_steps,
                                            fctor_types&&... fctor_args)
    {
        ...
        {
            fctor::template step<method, not_implicit>(std::forward<fctor_types>(fctor_args)...);
        }
    }
};
            ...
            for (auto m = 0; m < n_intervals - k + 1; ++m)
            {
                ...
                if (m == 0 || k == max_iterations)
                {
                    ...
                    subinterval[m]->template propagator<iter_type::fine_last>(
                      time + m * interval_length, fine_sol[m], f, k, n_finesteps,
...
#pragma omp ordered
                {
                    ...
                    subinterval[m]->template propagator<iter_type::coarse>(
                      time + (m + 1) * interval_length, coar_sol[m], f, k, n_coarsesteps,
