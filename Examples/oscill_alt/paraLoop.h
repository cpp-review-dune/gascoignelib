 ...
 * [parareal-algorithm](https://en.wikipedia.org/wiki/Parareal) for parallelizing the
 * solution of PDEs using the FEM libary Gascoigne.
 *
 * # The algorithm
 * Parareal is an algorithm parallelizing the solution of PDEs in time using
 * a fine and a coarse propagator which operate on a time discretization. This decomposes
 * the time interval into multiple subintervals, where each of these are assigned to one
 * thread. The coarse propagator \f$G\f$ is used for predicting values at the end of each
 * subinterval which the fine propagator \f$F\f$ uses to correct the solution.
 * ### Sketch of the algorithm
 * 1. Initialization
 *
 *    Initialize values on each subinterval \f$i = 0,\dots,\,N-1\f$ with the coarse
 * propagator: \f[g_{i} \longleftarrow G(t_{i},\,t_{i-1},\,g_{i-1})\f] Set iteration
 * counter \f$k = 1\f$
 * 2. Parallelization
 *
 ...
 * correct definition of the problem, there is no checking. The implementation only
 * executes the bare algorithm. Checking for convergence is not available at the moment.
 * This can be done via comparing serial with parallel execution. Some properties
 ...
 *
 * Every Instance of parareal correponds to one solver on an subinterval. Thus every
 * solver also has its own map of vectors. The user does not have to interact directly
 * with them. He only needs to call runPara or compare_para_serial, everything else is
 * handled automatically by the class. For better debugging one can specify the `PDBG`
 * flag (i.e. `-PDBG=n`), where`n` can be 1 or something arbitrary larger. In the case
 * where `n=1` only some more timings are taken. Otherwise additional visualization will
 * be written.
 */
template <int DIM>
...
public:
   ...
   *
   * Every instance needs the index of the subinterval to identify the vectors of the
   * fine, coarse and final solution internally \param id index of parareal instance which
   ...
   *
   * \param ostrm std::ostream where data should be written; cerr by default but can also
   * be a file (ofstream)
   */
   ...
   * parareal_algorithm afterwards to execute parareal algorithm. If no parameters are
   * given, the parameters will be read from the paramfile. Only parameters which may be
   * varied in some tests are included. \param max_iterations maximal number of iterations
   * executed by parareal algorithm \param coarse_theta \f$\theta\f$ for coarse time
   * stepping \param dtcoarse coarse time step size \param theta \f$\theta\f$ for fine
   * time stepping \param dtfine fine time step size
   */

  static double runPara(const int    max_iterations= 1,
                        const double coarse_theta  = get_param<double>("thetacoarse",
                                                                     "Equation"),
   ...
   * thetas, values for fine method is taken from the paramfile. Before execution of
   * parareal it solves the problem sequentially and calculates the norm \f$l_2\f$ vector
   * norm between the two solutions on every time node. \param max_iterations maximal
   * number of iterations executed by parareal algorithm \param dtcoarse_vec Vector with
   * coarse time step sizes that will be tested against sequential execution \param
   * coarse_theta_vec Vector with coarse theta values for step sizes specified by
   * dtcoarse_vec
   */
  ...
  static ParamFile paramfile;       //!< Name of the file in which all problem-related
...
private:
  /*!
   * \brief Wrapper function for GetSolver calls via GetMultiLevelSolver returning
   * *nonconst* pointer to Solver
   */
   ...
   * fine_last. \param time          current time from where the steps are executed \param
   * u VectorInterface with approximation at current time \param f             Right hand
   * inside
   */
  ...
  void propagator(double           time,
                  ...
                  VectorInterface& f,
                  const int        current       = 0,
                  const int        n_steps       = n_finesteps,
                  const int        implicit_steps= 0);

  template <iter_type method>
  inline void step(double&          time,
                   const double&    dt,
                   const int&       iter,
                   const double&    theta,
                   VectorInterface& u,
  ...
  inline void divergence_stab(VectorInterface& u,
                              VectorInterface& f,
                              const double&    time,
                              const double&    dt,
                              const int&       current);
   ...
   *
   * Every Vector is represented by a VectorInterface (a string) which itself is just a
   * key in a map associated to a solver. If one wants to change the Vector itself it does
   * not suffice to copy the VectorInterface but one rather has to change the value in the
   * map. \param v Key in the map associated with the solver \param v_ptr Value in the map
   ...
   *
   * The method calculates the correct size of a GlobalVector with parameters of the
   * solver. \param v_ptr Adress of the GlobalVector to be modified
   */
   ...
   *
   * The method sets the number of the subintervals to an already defined number of
   * threads (with `omp_get_thread_num()`), or to the maximum number of threads available
   * with `omp_get_max_threads()`. Setting the number of subintervals to the maximum is
   * good for using the maximum potential of parallelization. If threads are defined in
   * the paramfile, they are used.
   */
  ...
  static const std::string
    get_name(double dtc= dtcoarse, double tc= coarse_theta, int max_iter= max_iterations);

  const std::size_t subinterval_idx;  //!< Identifies the subinterval on which the loop is
                                      //!< acting in the first iteration
  VectorInterface
...
template <int DIM>
double parareal<DIM>::runPara(const int    max_iterations,
                              const double coarse_theta,
                              const double dtcoarse,
                              const double fine_theta,
                              const double dtfine) {
  ...
  assert(isfinite(n_coarsesteps) && isfinite(n_finesteps)
  ...
  assert(dtcoarse * n_coarsesteps - dtfine * n_finesteps
...
template <int DIM>
double parareal<DIM>::parareal_algorithm(const int n_intervals,
  ...
  static auto set_coar_weight= [](parareal* const& para_solver,
                                  VectorInterface& f,
                                  parareal* const& para_coarse,
                                  VectorInterface& c,
                                  double           damping) {
    nvector<double> c_c(2 * DIM + 1, 0);
    nvector<double> f_f(2 * DIM + 1, 0);
    nvector<double> c_f(2 * DIM + 1, 0);
    para_coarse->GetSolver()->GetGV(c).ScalarProductComp(
      c_c, para_coarse->GetSolver()->GetGV(c));
    para_solver->GetSolver()->GetGV(f).ScalarProductComp(
      f_f, para_solver->GetSolver()->GetGV(f));
    para_solver->GetSolver()->GetGV(f).ScalarProductComp(
      c_f, para_coarse->GetSolver()->GetGV(c));
