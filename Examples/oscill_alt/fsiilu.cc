...
extern StopWatch _sm_ns,_sm_solid,_sm_ext;
  ...
  {
    const FSIMatrix<DIM>* SM = 
      dynamic_cast<const FSIMatrix<DIM>* > (&__XXX);
    assert(SM);
    ...
    _xs1.ncomp()=DIM; _xs1.resize(SM->GetS11().n());
    _ys1.ncomp()=DIM; _ys1.resize(SM->GetS11().n());
    _hs1.ncomp()=DIM; _hs1.resize(SM->GetS11().n());

    _xs2.ncomp()=DIM; _xs2.resize(SM->GetS22().n());
    _ys2.ncomp()=DIM; _ys2.resize(SM->GetS22().n());
    _hs2.ncomp()=DIM; _hs2.resize(SM->GetS22().n());

    _xf_ns.ncomp()=DIM+1; _xf_ns.resize(SM->GetF_NS().n());
    _hf_ns.ncomp()=DIM+1; _hf_ns.resize(SM->GetF_NS().n());
    _yf_ns.ncomp()=DIM+1; _yf_ns.resize(SM->GetF_NS().n());
    _xf_ext.ncomp()=DIM;  _xf_ext.resize(SM->GetF_EXT().n());
    _yf_ext.ncomp()=DIM;  _yf_ext.resize(SM->GetF_EXT().n());
    _hf_ext.ncomp()=DIM;  _hf_ext.resize(SM->GetF_EXT().n());
  }
  ...
  {
    const FSIMatrix<DIM>* SM = 
      dynamic_cast<const FSIMatrix<DIM>* > (A);
    assert(SM);
    ...
    const ColumnDiagStencil* SS = 
      dynamic_cast<const ColumnDiagStencil*> (s11.GetStencil());
	...
	for (int p = SS->start(r);p<SS->stop(r);++p)
	  {
		...
		double sc = b11(c1,c1);
		if (sc!=0) 
		  assert((fabs(dx/sc)<1.e-7)||
			 ((b11(c1,c1)==1)&&(b21(c1,c1)==0)));
	      }
	  }
    ...
    if (1)
      {
    	...
    	const ColumnDiagStencil* ST = dynamic_cast<const ColumnDiagStencil*> (__F_NS.GetStencil());
    	assert(ST);
    ...
    if (1)
      {
    	...
    	const ColumnDiagStencil* ST = dynamic_cast<const ColumnDiagStencil*> (__F_EXT.GetStencil());
    	assert(ST);
  ...
  void FSIIlu<DIM>::compute_ilu ()
  {
    ...
    S.start();
    __F_NS.compute_ilu();
    S.stop(); cout << "NS: " << S.read() << endl; S.reset(); S.start();
    __F_EXT.compute_ilu();
    S.stop(); cout << "EX: " << S.read() << endl; S.reset(); S.start();
    __SM.compute_ilu();
    S.stop(); cout << "SM: " << S.read() << endl; S.reset(); S.start();
    __SES.compute_ilu();
    S.stop(); cout << "SE: " << S.read() << endl; 
  }
  ...
  template<int DIM>
  void FSIIlu<DIM>::Solid1_l2g_add(GlobalVector& x, const GlobalVector& xs,double s) const
  {
    assert(xs.n()==__solid_l2g->size());
    assert(xs.ncomp()==DIM);
    int l=0;
  ...
  template<int DIM>
  void FSIIlu<DIM>::Solid2_l2g_add(GlobalVector& x, const GlobalVector& xs,double s) const
  {
    assert(xs.n()==__solid_l2g->size());
    assert(xs.ncomp()==DIM);
    int l=0;
  ...
  template<int DIM>
  void FSIIlu<DIM>::Fluid_NS_l2g_add(GlobalVector& x, const GlobalVector& xf, double s) const
  {
    assert(xf.n()     == __fluid_l2g->size());
    assert(xf.ncomp() == DIM+1);
    int l=0;
  ...
  template<int DIM>
  void FSIIlu<DIM>::Fluid_EXT_l2g_add(GlobalVector& x, const GlobalVector& xf, double s) const
  {
    assert(xf.n()     == __fluid_l2g->size());
    assert(xf.ncomp() == DIM);
    assert(xf.ncomp()+DIM+1==x.ncomp());
  ...
  template<int DIM>
  void  FSIIlu<DIM>::Jacobi(const MatrixInterface &A, 
  ...
  void FSIIlu<DIM>::solve_fluid(GlobalVector& r) const
  {
	...
	for (int it=0;it<4;++it)
	  {
	    hf_ext = _yf_ext;
	    if (it>0)
	      vmult_NS_dirichlet(__FSIMATRIX->GetF_EXT(),
				 hf_ext,_xf_ext,-1.0);
	...
	int maxiter = 12;
	double tol = 1.e-3;
	maxiter = CG(__FSIMATRIX->GetF_EXT(),
		     _xf_ext,_yf_ext,
		     __F_EXT, maxiter,tol);
      }
	...
	for (int iter=0;iter<maxiter;++iter)
	  {
	    _hf_ns = _yf_ns;
	    vmult_NS_dirichlet(__FSIMATRIX->GetF_NS(),
	     		       _hf_ns,_xf_ns,-1.0);
  ...
  int FSIIlu<DIM>::BiCGSTAB(const MatrixInterface &A, 
			    GlobalVector &x, const GlobalVector &b,
			    const IluInterface& M, int &max_iter, double &tol)const
  {
    int n = x.n();
    int ncomp = x.ncomp();
    double  resid;
    double rho_1, rho_2, alpha, beta, omega;
    GlobalVector p(ncomp,n), phat(ncomp,n), s(ncomp,n), shat(ncomp,n), t(ncomp,n), v(ncomp,n);
  ...
  template<int DIM>
  void FSIIlu<DIM>::mg_smooth_ses(GlobalVector& x,GlobalVector& y, GlobalVector& h, int iter) const
  {
  ...
  void FSIIlu<DIM>::mg_ses(GlobalVector& x,GlobalVector& y, GlobalVector& h) const
  {
	...
	double tol=1.e-4;
	x.zero();	
	BiCGSTAB(__FSIMATRIX->GetS22(),
		 __FSIMATRIX->GetS12(),
		 x,y, __SES,maxiter,tol);
      }
    else
      {
	assert(thislevel>0);
	...
	X.ncomp() = Y.ncomp();
	X.resize(Y.n()); X.zero();
	H.ncomp() = Y.ncomp(); H.resize(Y.n()); H.zero();
