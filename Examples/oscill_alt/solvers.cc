...
template <int DIM>
DiscretizationInterface* FSISolver<DIM>::NewDiscretization(int           dimension,
...
template <>
void FSISolver<2>::reinit_element(int                             en,
...
template <>
void FSISolver<3>::reinit_element(int                             en,
                                  const nvector<int>&             indices,
                                  HASHMAP<int, std::vector<int>>& solid_interface_cells,
                                  HASHMAP<int, std::vector<int>>& fluid_interface_cells,
                                  HASHSET<int>&                   fluid_cells,
...
void FSISolver<DIM>::ReInitInterface(AleBaseDiscretization* ALEDISC) {
  ...
  // Nodes Fluid & Solid,  local <-> global (fluid nodes include interface and same for
  // solid)
  fluid_l2g.clear();
  solid_l2g.clear();
  fluid_g2l.clear();
