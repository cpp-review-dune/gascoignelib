...
extern "C" long umfpack_dl_symbolic(long         n,
                                    long         m,
                                    const long   Ap[],
                                    const long   Ai[],
                                    const double Ax[],
...
extern "C" long umfpack_dl_numeric(const long   Ap[],
                                   const long   Ai[],
                                   const double Ax[],
                                   void*        Symbolic,
                                   void**       Numeric,
                                   const double Control[UMFPACK_CONTROL],
                                   double       Info[UMFPACK_INFO]);
extern "C" long umfpack_dl_solve(long         sys,
                                 const long   Ap[],
                                 const long   Ai[],
                                 const double Ax[],
                                 double       X[],
...
extern "C" long umfpack_dl_triplet_to_col(long         n,
                                          long         nz,
                                          const long   Ti[],
                                          const long   Tj[],
                                          const double Tx[],
...
extern "C" void umfpack_dl_report_control(const double Control[UMFPACK_CONTROL]);
extern "C" long umfpack_dl_report_symbolic(const char   name[],
                                           void*        Symbolic,
                                           const double Control[UMFPACK_CONTROL]);
extern "C" long umfpack_dl_report_numeric(const char   name[],
                                          void*        Numeric,
                                          const double Control[UMFPACK_CONTROL]);
extern "C" void umfpack_dl_report_control(const double Control[UMFPACK_CONTROL]);
...
template <class B>
UmfSplit<B>::UmfSplit()
  : __AS(0), Control(NULL), Info(NULL), Symbolic(NULL), Numeric(NULL) {
  // __ncomp = 0;
...
void UmfSplit<B>::ConstructStructureFluid(const MatrixInterface& A,
  ...
  int status= umfpack_dl_symbolic(n_umf, n_umf, sb, cb, NULL, &Symbolic, Control, Info);
...
template <class B>
void UmfSplit<B>::ConstructStructureSolid(const MatrixInterface& A,
...
void UmfSplit<B>::copy_entries_fluid(const MatrixInterface* A,
  ...
  for (int r= 0; r < __Ap.size() - 1; ++r) {
    ...
    //	int comp = r%__ncomp;
    // fluid -> do nothing
    if (fluid_nodes.find(row) != fluid_nodes.end())
      continue;
  ...
  for (int r= 0; r < __Ap.size() - 1; ++r) {
    ...
    // fluid -> do nothing
    if (interface_nodes.find(row) == interface_nodes.end())
      continue;
...

{
  ...
  for (int r= 0; r < __Ap.size() - 1; ++r) {
    ...
    //	int comp = r%__ncomp;
    // solid -> do nothing
    if (solid_nodes.find(row) != solid_nodes.end())
      continue;
