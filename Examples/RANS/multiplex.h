...
template <int DIM>
void inline init_strain_rot(Eigen::Matrix<double, DIM, DIM>& strain,
                            Eigen::Matrix<double, DIM, DIM>& rotation, const FemFunction& U)
{
    assert(0);
}

template <>
void inline init_strain_rot<2>(Eigen::Matrix<double, 2, 2>& strain,
                               Eigen::Matrix<double, 2, 2>& rotation, const FemFunction& U)
{
    strain << U[1].x(), 0.5 * (U[1].y() + U[2].x()), 0.5 * (U[1].y() + U[2].x()), U[2].y();
    rotation << 0, 0.5 * (U[1].y() - U[2].x()), 0.5 * (U[2].x() - U[1].y()), 0;
}

template <>
void inline init_strain_rot<3>(Eigen::Matrix<double, 3, 3>& strain,
                               Eigen::Matrix<double, 3, 3>& rotation, const FemFunction& U)
{
    strain << U[1].x(), 0.5 * (U[1].y() + U[2].x()), 0.5 * (U[1].z() + U[3].x()),
      0.5 * (U[1].y() + U[2].x()), U[2].y(), U[2].y(), 0.5 * (U[2].z() + U[3].y()),
      0.5 * (U[3].x() + U[1].z()), 0.5 * (U[3].y() + U[2].z()), U[3].z();
    rotation << 0, 0.5 * (U[1].y() - U[2].x()), 0.5 * (U[1].z() - U[3].x()),
      0.5 * (U[2].x() - U[1].y()), 0, 0.5 * (U[2].z() - U[3].y()), 0.5 * (U[3].x() - U[1].z()),
      0.5 * (U[3].y() - U[2].z()), 0;
}

template <int DIM>
void inline init_nP(Eigen::Matrix<double, DIM, 1>& nabla_p, const FemFunction& U)
{
    assert(0);
}

template <>
void inline init_nP<2>(Eigen::Matrix<double, 2, 1>& nabla_p, const FemFunction& U)
{
    nabla_p << U[0].x(), U[0].y();
}

template <>
void inline init_nP<3>(Eigen::Matrix<double, 3, 1>& nabla_p, const FemFunction& U)
{
    nabla_p << U[0].x(), U[0].y(), U[0].z();
}
