...
namespace Gascoigne
{
  ...
  void Loop<DIM>::run(const std::string& problemlabel)
  {
    ...
    func_log <<"cells\t" <<"nodes\t"<<"func\t\t\t"<<"newton\t"<<"GMRES\t"<<"GMRES_mean\t"<<"time"<<endl;
    clock_t t;
    ...
    cout<<"Computation on preref:"<<i<<endl;
		//// SOLVE
		  ...
		  func_log <<GetMeshAgent()->ncells() <<"\t"<<GetMeshAgent()->nnodes()<<"\t"<<juh<<"\t"<<newton_Steps<<"\t"<<GMRES_Steps<<"\t"<<GMRES_Steps/newton_Steps  <<"\t\t"<<((float)t)/CLOCKS_PER_SEC<<endl;
