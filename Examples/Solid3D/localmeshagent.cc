...
void LocalMeshAgent::BuildQ4PatchList(const IntVector &patchl2g) {
  ...
  if (patchl2g.size() != q2patch.size()) {
    cerr << "LocalMeshAgent::BuildQ4PatchList: patchl2g must be same size as "
            "q2patch!!!"
         << endl;
    abort();
  }
      ...
      for (IntVector::const_iterator p_col = _periodicCols.begin();
        ...
        for (IntVector::const_iterator p_first = iv_FirstBoundary.begin();
          ...
          if (i_dim == 2) {
            ...
            std::cerr << "AssemblePeriodicBoundaries funktioniert nur fuer "
                         "Dimension 2 und 3.\n";
            abort();
          }
          ...
          if (max_diff_bestfit > 1e-10 &&
            ...
            std::cerr << "Distance between boundaries " << col_v << " and "
                      << col_w << " too large. Check your PeriodicMapping."
                      << std::endl;
            abort();
          }
