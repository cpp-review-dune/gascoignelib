...
namespace Gascoigne
{
  LocalMeshAgent::LocalMeshAgent() : MeshAgentInterface(), _goc2nc(false), HMP(NULL), GMG(NULL)
{
...
void LocalMeshAgent::BuildQ4PatchList(const IntVector &patchl2g)
{
  ...
  {
    cerr << "LocalMeshAgent::BuildQ4PatchList: patchl2g must be same size as q2patch!!!" << endl;
    abort();
  }
...
void LocalMeshAgent::BasicInit(const string& gridname, int dim, int patchdepth, int epatcher, bool goc2nc)
{
  assert(HMP==NULL);
  _goc2nc = goc2nc;
  if (dim==2)
...
void LocalMeshAgent::write_gup(const string& fname,const GlobalVector& u) const
{
  assert(HMP);
