...
namespace Gascoigne
{
 ...
 template<int DIM>
  void Solid<DIM>::point(double h, const FemFunction& U, const Vertex<DIM>& v) const
  {
    //__h = h;
    //__v = v;
    ...
    if(mat_law=="STVK")
    {
	  ...
	  {
	    C =F.transpose()*F;
	  	SIGMAs = mu_s*pow(J,-2.0/3.0)*(MATRIX::Identity()-1.0/3.0*C.trace()*C.inverse())+0.5*kapa_s*(pow(J,2.0)-1.0)*C.inverse();
	  }
	  else abort();
  ...
  template<int DIM>
  void Solid<DIM>::Form(VectorIterator b, const FemFunction& U, const TestFunction& N) const
  {
  //phi =nabla N;
    VECTOR phi; multiplex_init_test<DIM>(phi,N);
   
		// FULL tensor F Sigma
		for (int i=0;i<DIM;++i) b[i] += (F*SIGMAs*phi)(i,0);
      
  }
  

  template<int DIM>
  void Solid<DIM>::Matrix(EntryMatrix& A, const FemFunction& U, const TestFunction& M, const TestFunction& N) const
  {    
 ...
 template<int DIM>
 void Solid<DIM>::point_M(int j, const FemFunction& U, const TestFunction& M) const
  {
    VECTOR psi; multiplex_init_test<DIM>(psi,M);
    if(mat_law=="STVK")
    {
	  ...
	  else if(mat_law=="artery")
	  {
			...
			for (int j=0;j<DIM;++j)
				{
					MATRIX C_dU=  (psi*(F.block(j,0,1,DIM))+F.block(j,0,1,DIM).transpose() * psi.transpose());
					MATRIX C_dU_inverse= - C.inverse()*C_dU*C.inverse();
					double J_dU=J*(F.inverse().block(0,j,DIM,1)*psi.transpose()).trace();
					
					SIGMA_dU[j] = F*mu_s*(-2.0/3.0)*pow(J,-5.0/3.0)*J_dU*(MATRIX::Identity()-1.0/3.0*C.trace()*C.inverse());
					SIGMA_dU[j] +=F*mu_s*pow(J,-2.0/3.0)*(-1.0/3.0*C_dU.trace()*C.inverse());
					SIGMA_dU[j] +=F*mu_s*pow(J,-2.0/3.0)*(-1.0/3.0*C.trace()*C_dU_inverse);
  ...
  template<int DIM>
  void Solid<DIM>::MatrixBlock(EntryMatrix& A, const FemFunction& U, const FemFunction& N) const
  {;
