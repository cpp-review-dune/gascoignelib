...
protected:
  ...
  const SplittingSolver *GetSplittingSolver() const {
    const SplittingSolver *S =
        dynamic_cast<const SplittingSolver *>(GetSolver());
    assert(S);
    return S;
  }
  ...
  void ChorinUzawa();
  void VanKan();
  void VelocityPredictor(VectorInterface &v, VectorInterface &fv,
                         NLInfo &nlinfo, int iter);
  void PressurePoissonProblem(VectorInterface &q, VectorInterface &fp,
                              CGInfo &cginfo);
...
public:
  ChorinAlgorithm()
      : NonstationaryAlgorithm(), niter(0), initial(""), pproblem(""),
        vproblem("") {}
