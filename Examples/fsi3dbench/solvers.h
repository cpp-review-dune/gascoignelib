...
private:
  ...
  //    mutable FSISparseUmf<FMatrixBlock<DIM+1> >      _ILU_F,_ILU_S;
  mutable FSISparseUmf<FMatrixBlock<DIM + 1>> _ILU_S;
  mutable FSISparseBlockIlu<FMatrixBlock<DIM + 1>> _ILU_F;
...
public:
  ...
  void UpdateDeformation(Vector& x);

  double NewtonUpdate(double& rr,
                      Vector& x,
                      Vector& dx,
                      Vector& r,
                      const Vector& f,
