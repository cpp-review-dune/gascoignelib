...
namespace Gascoigne
{
  ...
  template <int DIM>
  void FSISolver<DIM>::Form(VectorInterface &y,
  ...
  template <int DIM>
  void FSISolver<DIM>::smooth(int niter,
                              VectorInterface &x,
  ...
  template <int DIM>
  void FSISolver<DIM>::smooth_exact(VectorInterface &x,
                                    const VectorInterface &y,
...
//  passiert"<<endl; abort();}
  ...
  template <int DIM>
  void FSISolver<DIM>::reinit_element(int en,
                                      const nvector<int> &indices,
  ...
  void FSISolver<DIM>::ReInitInterface(AleBaseDiscretization *ALEDISC)
  {
    ...
    if (dim == 2)
    {
      const GascoigneMesh2d *M =
          dynamic_cast<const GascoigneMesh2d *>(GetMesh());
      assert(M);
    ...
    {
      const GascoigneMesh3d *M =
          dynamic_cast<const GascoigneMesh3d *>(GetMesh());
      assert(M);
  ...
  template <>
  void FSISolver<2>::PointVisu(const string &name,
  ...
  template <>
  void FSISolver<3>::PointVisu(const string &name,
                               const GlobalVector &u,
                               int iter) const
  {
    VectorInterface def("def");
  ...
  template <int DIM>
  double FSIMultiLevelSolver<DIM>::NewtonUpdate(double &rr,
                                                VectorInterface &x,
                                                VectorInterface &dx,
                                                VectorInterface &r,
