...
namespace Gascoigne {
template <int DIM> FSISolver<DIM>::FSISolver() : StdSolver() {}
...
template <int DIM> void FSISolver<DIM>::ReInitExtensionMatrix() {
...
template <int DIM> void FSISolver<DIM>::ComputeSAI() const {
  if (_directsolver)
    return;
  ...
  const SparseBlockMatrix<FMatrixBlock<DIM + 1>> &A =
      dynamic_cast<const SparseBlockMatrix<FMatrixBlock<DIM + 1>> &>(
          *GetMatrix());
...
//////// deformation update
template <int DIM> void FSISolver<DIM>::SolveExtension(VectorInterface &x) {
...
template <int DIM>
double FSIMultiLevelSolver<DIM>::NewtonUpdate(double &rr, VectorInterface &x,
                                              VectorInterface &dx,
                                              VectorInterface &r,
