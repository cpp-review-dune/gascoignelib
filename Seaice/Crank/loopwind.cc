...
for (int T=0;T<=1;++T)
     {
       GlobalVector& U = GetMultiLevelSolver()->GetSolver()->GetGV(u);
	  ...
	  U(n,0) = (0.1*(2*1.e6*M->vertex2d(n).y()-1.e6*0.5)/(1.e6*0.5))/10;
	  U(n,1) = (-0.1*(2*1e6*M->vertex2d(n).x()-1.e6*0.5)/(1.e6*0.5))/10;
   	}
      GetMultiLevelSolver()->GetSolver()->Visu("wasser",u,T);
    }
   abort();
  ...
  for (int T=0;T<=32;++T)
     {
       ...
       for (int n=0;n<M->nnodes();++n)
   	{
  	  U(n,0) = WWindX(1000.0*M->vertex2d(n).x(),1000.0*M->vertex2d(n).y(),T);
   	  U(n,1) = WWindY(1000.0*M->vertex2d(n).x(),1000.0*M->vertex2d(n).y(),T);
   	}
      GetMultiLevelSolver()->GetSolver()->Visu("wind",u,T);
    }
   abort();
*/
  cout << "------------------------------" << endl;
  ...
  for (_iter=1; _iter<=_niter; _iter++)
      ...
      if(stepback>0.0)
	{
      ...
      if(DT<0.125)
	{ cout<<"Zeitschritt zu klein" <<endl;
	  abort();}
      ...
      if (TIME>endtime*24*60*60/tref)
	{
      ...
      // Transoprtgleichung 
      GetMultiLevelSolver()->GetSolver()->Equ(oldh,1.0, hl);
      int NSUB = 20;
      for (int sub=0;sub<NSUB;++sub)
	{
      ...
      GetMultiLevelSolver()->AddNodeVector("H", hl);
