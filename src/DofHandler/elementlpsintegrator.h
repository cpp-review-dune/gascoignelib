...
public:
            ...
            const LocalData& Q, const LocalData& QC) const
  {
    ...
    for (int k = 0; k < IF.n(); k++)
    {
      ...
      for (int i = 0; i < FEM.n(); i++)
      {
        LEQ.StabForm(F.start(i), ElementIntegrator<DIM, IFF, IFE, IFB, IFM>::_UH, UHP, MLPS[i]);
      }
    }
  }
...
#define ElementLpsIntegratorQ12d                                                        \
  ElementLpsIntegrator<2, PatchFormula2d<4, QuadGauss4>, PatchFormula2d<9, QuadGauss9>, \
...
#define ElementLpsIntegratorQ13d                                                        \
  ElementLpsIntegrator<3, PatchFormula3d<8, HexGauss8>, PatchFormula3d<27, HexGauss27>, \
