...
namespace Gascoigne
{
  ...
  public:
    ...
    const CGDofHandler<DIM,M>& GetDofHandler() const
    {
      return dofhandler;
    }
    ...
    {
      return "Lagrange Discretization";
    }
    ...
    int ndofs() const
    {
      return GetDofHandler().ndofs();
    }
    int nelements() const
    {
      return GetDofHandler().nelements();
    }
    ...
    Vertex2d vertex2d(int i) const { return GetDofHandler().vertex2d(i); }
    Vertex3d vertex3d(int i) const { return GetDofHandler().vertex3d(i); }
    ...
    void Transformation(nmatrix<double> &T, int iq) const
    {
      ...
      int ne = GetDofHandler().dofs_per_element();

      IntVector indices = GetDofHandler().GetElement(iq);
    ...
    void Structure(SparseStructureInterface *SI) const
    {
      ...
      S->build_begin(ndofs());
      for (int iq = 0; iq < GetDofHandler().nelements(); iq++)
	{
	  IntVector indices = GetDofHandler().GetElement(iq);
	  // HANGING NODES
...
#pragma omp parallel private(T,finiteelement,integrator, __U,__F,__QN,__QC)
	...
	for(auto col : COLS)
	  {
	    const IntVector& q = *GetDofHandler().ElementOnBoundary(col);
	    const IntVector& l = *GetDofHandler().ElementLocalOnBoundary(col);
...
#define LagrangeDiscQ12d LagrangeDisc<2,2,FiniteElement<2, 1, CGTransformationQ12d, CGBaseQ12d>, ElementIntegratorQ12d >
#define LagrangeDiscQ22d LagrangeDisc<2,3,FiniteElement<2, 1, CGTransformationQ22d, CGBaseQ22d>, ElementIntegratorQ22d >
#define LagrangeDiscQ42d LagrangeDisc<2,5,FiniteElement<2, 1, CGTransformationQ42d, CGBaseQ42d>, ElementIntegratorQ42d >
...
#define LagrangeDiscQ13d LagrangeDisc<3,2,FiniteElement<3, 2, CGTransformationQ13d, CGBaseQ13d>, ElementIntegratorQ13d >
#define LagrangeDiscQ23d LagrangeDisc<3,3,FiniteElement<3, 2, CGTransformationQ23d, CGBaseQ23d>, ElementIntegratorQ23d >
#define LagrangeDiscQ43d LagrangeDisc<3,5,FiniteElement<3, 2, CGTransformationQ43d, CGBaseQ43d>, ElementIntegratorQ43d >
