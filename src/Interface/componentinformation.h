...
protected:
    ...
    mutable const ProblemDescriptorInterface* PDI;
...
public:
    ...
    virtual void SetProblemDescriptor(const ProblemDescriptorInterface& PDX) const
    {
        PDI = &PDX;
        assert(PDI);
    };

    const ProblemDescriptorInterface* GetProblemDescriptorInterface() const
    {
        assert(PDI);
        return PDI;
    };
