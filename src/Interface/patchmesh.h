...
class PatchMesh : public MeshInterface
{
 ...
 public:
  ...
  virtual IntVector  CoarseIndices(int iq)             const { assert(0);}
  virtual const IntVector& IndicesOfPatch(int i)         const { assert(0);}
  virtual const IntVector& VertexOnBoundary(int color) const { assert(0);}
