...
namespace Gascoigne
{
  ...
  /**
   * \brief Data class for the Navier-Stokes equations, stationary and non-stationary
   *
   * Provides the required persistent data to fully specify the Navier-Stokes  equations
   *  
  ...
  public:
    ...
    ////////////////////////////////////////////////// Correction of the the do-nothing boundary
    ...
    {
      if (data.symmetrictensor == false) // only correct outflow boundary if symmetric tensor is used
	return;
   ...
   */
  template <int DIM>
  class NavierStokesLpsTime : public virtual LpsEquation, public BoundaryEquation
  {
  ...
  public:
	  ...
	  if (data.theta<1)
	    {
	      // viscosity
	      for (int j=0;j<DIM;++j)
		b[i+1] += (1.0-data.theta) * data.visc * (*OLD)[i+1][j+1] * N[j+1];
	      
	      if (data.symmetrictensor)
		for (int j=0;j<DIM;++j)
		  b[i+1] += (1.0-data.theta) * data.visc * (*OLD)[j+1][i+1] * N[j+1];
	      
	      // convection
	      for (int j=0;j<DIM;++j)
		b[i+1] += (1.0-data.theta) * (*OLD)[j+1].m() * (*OLD)[i+1][j+1] * N.m();
	    }
	}
    }
      ...
      if (col == data.outflowcolor)
	{
	  for (int i=0;i<DIM;++i)
	    for (int j=0;j<DIM;++j)
	      {
		b[i+1] -= data.theta       * data.visc * normal[j] * U[j+1][i+1] * N.m();
		b[i+1] -= (1.0-data.theta) * data.visc * normal[j] * (*OLD)[j+1][i+1] * N.m();
	      }
	}
    }
      ...
      if (col == data.outflowcolor)
	{
	  for (int i=0;i<DIM;++i)
	    for (int j=0;j<DIM;++j)
	      A(i+1,j+1) -= data.theta * data.visc * normal[j] * M[i+1] * N.m();
	}
    }
