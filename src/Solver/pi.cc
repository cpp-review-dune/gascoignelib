...
void Pi::vmult(CompVector<double>& y, const CompVector<double>& x, double s) const
{
  y.zero();
  int ncomp = x.ncomp();
  ...
  }
  {
    ...
    for (p = face.begin(); p != face.end(); p++)
    {
      ...
      for (int c = 0; c < ncomp; c++)
        y(i, c) = x(i, c) - 0.25 * (x(f[0], c) + x(f[1], c) + x(f[2], c) + x(f[3], c));
    }
  }
  {
    ...
    for (p = cell.begin(); p != cell.end(); p++)
    {
      ...
      for (int c = 0; c < ncomp; c++)
        y(i, c) = x(i, c)
                  - 0.125
                      * (x(f[0], c) + x(f[1], c) + x(f[2], c) + x(f[3], c) + x(f[4], c)
                         + x(f[5], c) + x(f[6], c) + x(f[7], c));
    }
  }
}
