...
namespace Gascoigne {
VecDirection::VecDirection(const DofHandlerBase* m)
{
  M = m;
  dimension = 0;
}
...
StreamDirection::StreamDirection(const DofHandlerBase* m,
...
StreamDirection::operator()(int i, int j) const
{
  if (dimension == 2) {
    ...
    double n = sqrt(a[0] * a[0] + a[1] * a[1]) *
               sqrt(X(i, dx) * X(i, dx) + X(i, dy) * X(i, dy));
    return (sc > 0.75 * n);
  } else {
    assert(dimension == 3);
