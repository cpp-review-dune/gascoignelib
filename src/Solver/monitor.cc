...
void Monitor::post_nonlinear(const DoubleVector& Ju, double eta, int ncells, int cg, int dcg)
{
    new_message.str("");
    new_message << "cg dnl: " << cg << " " << dcg << " [J(u)=" << Ju[0] << " eta=" << eta << "]";
...
void Monitor::PrintAscii(ostream& os, const string& s) const
{
    os << s;
}
}  // namespace Gascoigne
