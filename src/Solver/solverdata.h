...
public:
  ...
  const DoubleVector& GetIluModify() const
  {
    return _ilum;
  }
  ...
  const std::string& GetIluSort() const
  {
    return ilusort;
  }
  ...
  {
    return bicgstab_pstep;
  }
  std::string GetBiCGStabResidual() const
  {
    return bicgstab_residual;
  }
  ...
  {
    return bicgstab_miniter;
  }

  int GetCgMassMaxIter() const
  {
    return _cgMassMaxIter;
  }
