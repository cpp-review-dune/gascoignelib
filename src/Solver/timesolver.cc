...
extern Timer GlobalTimer; 
  ...
  void TimeSolver::Form(VectorInterface &gy,
  ...
  void TimeSolver::InverseMassMatrix(VectorInterface &u,
  ...
  void TimeSolver::cgvmult(VectorInterface &y,
  ...
  void TimeSolver::L2Projection(VectorInterface &Gu, VectorInterface &Gf)
  {
    ...
    while (!reached && iter < GetSolverData().GetCgMassMaxIter())
    {
      ...
      Res = r * r;
      // cout << "\t\tpcg " << iter << "\t" << sqrt(Res) << endl;
      if (Res < GetSolverData().GetCgMassTol() *
                    GetSolverData().GetCgMassTol() * FirstRes ||
