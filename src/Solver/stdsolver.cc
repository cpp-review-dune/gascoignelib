...
/*-----------------------------------------*/
#ifdef __WITH_THREADS__
extern "C" void METIS_PartGraphRecursive(int *,
                                         int *,
                                         int *,
                                         int *,
                                         int *,
                                         ...
                                         int *,
                                         int *);
extern "C" void METIS_PartGraphKway(int *,
                                    int *,
                                    int *,
                                    int *,
                                    int *,
...
namespace Gascoigne
{
  ...
  void StdSolver::OutputSettings() const
  {
    ...
    if (GetFaceDiscretization())
      cout << "FaceDiscretization:       " << GetFaceDiscretization()->GetName()
           << endl;
    ...
    if (__n_threads > 1)
    {
      ...
      for (int p = 0; p < __n_threads; ++p)
        for (set<int>::const_iterator it = __thread_domain2node_set[p].begin();
             it != __thread_domain2node_set[p].end();
             ++it)
...
//
                            ...
                            const NumericInterface *NI)
  {
    ...
    DFH.insert("min_patches_per_thread",
               &__min_patches_per_thread,
    ...
    GetFaceDiscretizationPointer() =
        NewFaceDiscretization(dimension, _facediscname);
    assert(_ZP);
  ...
  FaceDiscretization *
  StdSolver::NewFaceDiscretization(int dimension, const string &facediscname)
  {
    ...
    if (dimension == 2)
    {
      ...
      {
        cerr << " Solver::NewDiscretization()\tunknown facediscname="
             << facediscname << endl;
        abort();
      }
    }
    else if (dimension == 3)
    {
      ...
      {
        cerr << "Solver::NewFaceDiscretization()\tunknown facediscname="
             << facediscname << endl;
        abort();
      }
    }
    ...
    {
      cerr << "Solver::NewFaceDiscretization()\tdimension must either 2 or 3"
           << endl;
      abort();
    }
  }
  ...
  void StdSolver::residualgmres(VectorInterface &gy,
  ...
  void StdSolver::vmulteq(VectorInterface &gy,
                          const VectorInterface &gx,
                          double d) const
  {
    _vm.start();
  ...
  void StdSolver::MatrixResidual(VectorInterface &gy,
                                 const VectorInterface &gx,
                                 const VectorInterface &gb) const
  {
    Equ(gy, 1., gb);
  ...
  void StdSolver::SetPeriodicVector(VectorInterface &gf) const
  {
    ...
    if (PD == NULL)
    {
      ...
      {
        cerr << "No PeriodicData given but PeriodicColors in ParamFile!"
             << endl;
        abort();
      }
      return;
  ...
  void StdSolver::SetPeriodicVectorZero(VectorInterface &gf) const
  {
         ...
         p_col != iv_PeriodicColors.end();)
    {
      ...
      *p_col++;
      ...
      const IntVector &iv_PeriodicComponents =
  ...
  void StdSolver::smooth_pre(VectorInterface &x,
                             const VectorInterface &y,
  ...
  void StdSolver::smooth_exact(VectorInterface &x,
                               const VectorInterface &y,
                               VectorInterface &help) const
  {
#ifdef __WITH_UMFPACK__
  ...
  void StdSolver::smooth_post(VectorInterface &x,
                              const VectorInterface &y,
                              VectorInterface &help) const
  {
  ...
  void StdSolver::Form(VectorInterface &gy,
                       const VectorInterface &gx,
                       double d) const
  {
    _re.start();
  ...
  void StdSolver::AdjointForm(VectorInterface &gy,
                              const VectorInterface &gx,
                              double d) const
  {
    GlobalVector &y = GetGV(gy);
  ...
  void StdSolver::SolutionInit(VectorInterface &Gu) const
  {
    ...
    const DomainInitialCondition *u0 =
        dynamic_cast<const DomainRightHandSide *>(
  ...
  void StdSolver::AssembleError(GlobalVector &eta,
                                      ...
                                      const Functional *FP)
  {
    ...
    const BoundaryFunctional *BFP =
        dynamic_cast<const BoundaryFunctional *>(FP);
    const ResidualFunctional *RFP =
        dynamic_cast<const ResidualFunctional *>(FP);
  ...
  void StdSolver::Rhs(VectorInterface &gf, double d) const
  {
    ...
    const BoundaryRightHandSide *NRHS =
        GetProblemDescriptor()->GetBoundaryRightHandSide();
    ...
    if (NRHS)
    {
      ...
      GetDiscretization()->BoundaryRhs(
          f, BM->GetBoundaryRightHandSideColors(), *NRHS, d);
    }
  ...
  void StdSolver::DirichletMatrixOnlyRow() const
  {
    ...
    for (IntSet::const_iterator p = Colors.begin(); p != Colors.end(); p++)
    {
      int col = *p;
      const IntVector &comp = BM->GetDirichletDataComponents(col);
      GetDiscretization()->StrongDirichletMatrixOnlyRow(
          *GetMatrix(), col, comp);
    }
  }
    ...
    if (__n_threads > 1)
    {
      ...
      if (GetSolverData().GetIluSort() == "cuthillmckee")
      {
        ...
        for (int d = 0; d < __n_threads; d++)
        {
          ...
          cmc.Permutate(
              perm[d], __thread_domain2node[d], __thread_node2domain, d);
        }
        // Wie das?
        ThreadIlu *TIlu = dynamic_cast<ThreadIlu *>(GetIlu());
        assert(TIlu);
  ...
  void StdSolver::Equ(VectorInterface &dst,
                      double s,
  ...
  void StdSolver::Add(VectorInterface &dst,
                      double s,
                      const VectorInterface &src) const
  {
