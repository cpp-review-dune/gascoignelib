...
/*-----------------------------------------*/
#ifdef __WITH_THREADS__
extern "C" void METIS_PartGraphRecursive(int*, int*, int*, int*, int*, int*, int*, int*, int*, int*,
                                         int*);
extern "C" void METIS_PartGraphKway(int*, int*, int*, int*, int*, int*, int*, int*, int*, int*,
                                    int*);
#endif
/*-----------------------------------------*/
...
void StdSolver::_check_consistency(const Equation* EQ, const DiscretizationInterface* DI) const
{
    ...
    if (glseq && !glsdi)
    {
        cerr << "Warning: Discretization \"" << eq << "\" doesn't go with type of given Equation!"
             << endl;
    }
    else if (!glseq && glsdi)
    {
        cerr << "Error: Discretization \"" << eq << "\" doesn't go with type of given Equation!"
             << endl;
        abort();
    }
...
void StdSolver::ThreadPartitionMesh()
{
    ...
    if (__n_threads > 1)
    {
        ...
        if (__n_threads < 8)
            METIS_PartGraphRecursive(&__n, &adj[0], &adj1[0], NULL, NULL, &wgtflag, &numflag,
                                     &__n_threads, &options[0], &edgecut, &patch_partition[0]);
        else
            METIS_PartGraphKway(&__n, &adj[0], &adj1[0], NULL, NULL, &wgtflag, &numflag,
                                &__n_threads, &options[0], &edgecut, &patch_partition[0]);
                          ...
                          const NumericInterface* NI)
{
    ...
    {
        cerr << "Thread Ilu (with_thread_ilu) not available for matrixtype " << _matrixtype << "!"
             << endl;
...
DiscretizationInterface* StdSolver::NewDiscretization(int dimension, const string& discname)
{
    ...
    if (dimension == 2)
    {
        if (discname == "Q1")
            ...
            return new Q1Gls2d;
        else if (discname == "Q2Gls")
            return new Q2Gls2d;
        else if (discname == "Q1Lps")
            return new Q1Lps2d;
    ...
    else if (dimension == 3)
    {
        if (discname == "Q1")
            ...
            return new Q23dWithSecond;
        else if (discname == "Q2LpsWithSecond")
            return new Q2Lps3dWithSecond;
        else
        {
            cerr << " Solver::NewDiscretization()\tunknown discname=" << discname << endl;
            abort();
...
{
    if (facediscname == "none")
        return NULL;

    if (dimension == 2)
    {
        if (facediscname == "FaceQ1")
            return new FaceQ1<2>;
        else if (facediscname == "FaceQ2")
            return new FaceQ2<2>;
        else
        {
    ...
    else if (dimension == 3)
    {
        ...
        {
            cerr << "Solver::NewFaceDiscretization()\tunknown facediscname=" << facediscname
                 << endl;
            abort();
        }
    }
...
void StdSolver::SetBoundaryVectorStrong(VectorInterface& gf, const BoundaryManager& BM,
                                        const DirichletData& DD, double d) const
{
    GlobalVector& f = GetGV(gf);

    IntSet PrefCol = DD.preferred_colors();
    list<int> colors(BM.GetDirichletDataColors().begin(), BM.GetDirichletDataColors().end());
...
void StdSolver::SetPeriodicVectorStrong(VectorInterface& gf, const BoundaryManager& BM,
                                        const PeriodicData& PD, double d) const
{
    GlobalVector& f = GetGV(gf);

    list<int> periodic_cols(BM.GetPeriodicDataColors().begin(), BM.GetPeriodicDataColors().end());
...
void StdSolver::Form(VectorInterface& gy, const VectorInterface& gx, double d) const
{
    ...
    {
        const BoundaryManager* BM = GetProblemDescriptor()->GetBoundaryManager();
        GetDiscretization()->BoundaryForm(GetGV(gy), GetGV(gx), BM->GetBoundaryEquationColors(),
                                          *BE, d);
    }
    HNZero(gx);
    HNZeroData();
...
{
    GlobalVector& u = GetGV(Gu);
    const DomainInitialCondition* u0 =
      dynamic_cast<const DomainRightHandSide*>(GetProblemDescriptor()->GetInitialCondition());
    if (u0 == NULL)
    {
        u.zero();
        return;
...
void StdSolver::AssembleError(GlobalVector& eta, const VectorInterface& u, GlobalVector& err) const
{
    if (GetProblemDescriptor()->GetExactSolution() == NULL)
        return;
    HNAverage(u);
    GetDiscretization()->AssembleError(eta, GetGV(u), err,
...
double StdSolver::ComputeFunctional(VectorInterface& gf, const VectorInterface& gu,
                                    const Functional* FP)
{
...
double StdSolver::ComputeBoundaryFunctional(VectorInterface& gf, const VectorInterface& gu,
                                            VectorInterface& gz, const BoundaryFunctional* FP) const
{
    HNAverage(gu);
    HNAverageData();
    const BoundaryManager* BM = GetProblemDescriptor()->GetBoundaryManager();
...
double StdSolver::ComputeDomainFunctional(VectorInterface& gf, const VectorInterface& gu,
                                          VectorInterface& gz, const DomainFunctional* FP) const
{
    HNAverage(gu);
    HNAverageData();
...
double StdSolver::ComputePointFunctional(VectorInterface& gf, const VectorInterface& gu,
                                         VectorInterface& gz, const PointFunctional* FP) const
{
    HNAverage(gu);
    HNAverageData();
...
double StdSolver::ComputeResidualFunctional(VectorInterface& gf, const VectorInterface& gu,
                                            VectorInterface& gz, const ResidualFunctional* FP) const
{
    const BoundaryManager* BM = GetProblemDescriptor()->GetBoundaryManager();
...
void StdSolver::EvaluateCellRightHandSide(VectorInterface& f, const DomainRightHandSide& CF,
                                          double d) const
{
...
void StdSolver::EvaluateBoundaryCellRightHandSide(VectorInterface& f,
...
void StdSolver::EvaluateParameterRightHandSide(VectorInterface& f, const DomainRightHandSide& CF,
                                               double d) const
{
...
void StdSolver::EvaluateBoundaryParameterRightHandSide(VectorInterface& f,
                                                       const BoundaryRightHandSide& CF,
                                                       const BoundaryManager& BM, double d) const
{
    assert(f.GetType() == "parameter");
...
void StdSolver::AssembleMatrix(const VectorInterface& gu, double d)
{
    ...
    {
        const BoundaryManager* BM = GetProblemDescriptor()->GetBoundaryManager();
        GetDiscretization()->BoundaryMatrix(*GetMatrix(), u, BM->GetBoundaryEquationColors(), *BE,
                                            d);
    }
    PeriodicMatrix();
    DirichletMatrix();
...
void StdSolver::PermutateIlu(const VectorInterface& gu) const
{
    ...
    if (__n_threads > 1)
    {
        ...
        {
            std::cerr << "In StdSolver::PermutateIlu: IluSort " << GetSolverData().GetIluSort()
