...
namespace Gascoigne
{
StdTimeSolver::StdTimeSolver() : StdSolver(), _MMP(NULL), _dt(0.), _theta(0.), _time(0.)
{
  _rhs.fill(0.0);
}
...
void StdTimeSolver::SetTimeData(double dt, double theta, double time, double oldrhs,
                                double newrhs)
{
  _dt     = dt;
  _theta  = theta;
...
void Gascoigne::StdTimeSolver::InitialCondition(VectorInterface& gf, double d) const
{
  ...
  if (IC)
  {
    ...
    if (!done)
    {
      cerr << "InitialCondition should be either of type DomainRightHandSide or "
              "DiracRightHandSide!!!"
           << endl;
      abort();
    }
...
void StdTimeSolver::MassMatrixVector(VectorInterface& gf, const VectorInterface& gu,
                                     double d) const
{
  GlobalVector& f       = GetGV(gf);
  const GlobalVector& u = GetGV(gu);
                                    ...
                                    const TimePattern& TP, double s)
{
  ...
  while (!reached && iter < GetSolverData().GetCgMassMaxIter())
  {
    ...
    Res = r * r;
    cout << "\t\tpcg " << iter << "\t" << sqrt(Res) << endl;
    if (Res < GetSolverData().GetCgMassTol() * GetSolverData().GetCgMassTol() * FirstRes
