...
void CuthillMcKee::Permutate(IntVector& perm)
{
  ...
  //  int n = S->n();
  //  perm.resize(n);
  // sollte hier nicht aufgerufen werden, sondern ueberpruefen ob perm von der groesse her
  // stimmt
  idx_t n = S->n();
  assert(n == perm.size());
  ...
  for (int i = 0; i < n; ++i)
    Mperm[i] = perm[i];
  METIS_NodeND(&n, &adj[0], &adjncy[0], NULL, &options[0], &Mperm[0], &iperm[0]);
  for (int i = 0; i < n; ++i)
    perm[i] = Mperm[i];
...
void CuthillMcKee::Permutate(
  ...
  const std::vector<std::vector<std::pair<int, int>>>& node2domain, int domain)
{
  // mit metis graph aufbauen
  //
  // diese methode wird in StdSolver::PermutateIlu aufgerufen
