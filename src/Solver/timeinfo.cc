...
void TimeInfo::ReInit(double tb, double te, double det, const string& sch, int ne,
                      double tt)
{
  _tbegin = tb;
  _time   = _tbegin;
...
void TimeInfo::ReInitTheta()
{
  ...
  if (_actualscheme == "CN")
    _theta = 0.5;
  else if (_actualscheme == "Euler")
    _theta = 1.;
  else if (_actualscheme == "FractionalTheta")
    _theta = 0.;
  else if (_actualscheme == "Theta")
    _theta = _Theta;
}
