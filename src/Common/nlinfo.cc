...
NLInfo::check_damping(int dampit, double res)
{
  ...
  if (CD.residual() > 1.e20)
    return "exploded";
  if (!(CD.residual() <= 1.e20))
    return "exploded";

  if (CD.residual() <= CD.previousresidual())
    return "ok";
  return "continue";
}
...
NLInfo::check(int iter, double resi, double cori)
{
  ...
  if (CD.status() == "diverged")
    return 1;
  if (CD.status() == "stagnation")
    return 1;
  ...
  if (UD.printstep() && !(CD.iteration() % UD.printstep())) {
    ...
    cout.precision(3);
    cout << " " << lc << " [" << rr << "] {" << iter;
    if (Linfo.control().status() == "running")
      cout << "*";
    cout << "}" << endl;
  }
