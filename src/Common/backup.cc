...
ReadBackUp::ReadBackUp(GlobalVector& u, const string& name)
{
  ...
  if (u.n() != size) {
    cout << "Incompatibility u.n() size u.n()=" << u.n() << " file.n()=" << size
         << endl;
  }
  assert(u.n() == size);
  ...
  if (test != "BackUpEnd") {
    cout << "error in " << __FILE__ << ":" << __LINE__ << " : error, test=='"
         << test << "' should be =='BackUpEnd'" << endl;
    if (u.ncomp() != comp) {
      cout << "probably, because: expected comp nr=" << u.ncomp()
           << " NOT EQUAL the bup-file supplied comp nr=" << comp << endl;
    }
    abort();
  }
}
