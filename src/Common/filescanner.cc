...
FileScanner::readfile(const ParamFile& pf, const string& blockname)
{
  ...
  if (blockname == "Multilevelsolver") {
    std::cerr << "Error: Trying to search file '<< inputname <<' for Blockname "
                 "'Multilevelsolver'."
              << std::endl;
    std::cerr << "The blockname 'Multilevelsolver' is not used any longer! It "
                 "has changed to MultiLevelSolver."
              << std::endl;
  ...
  // der folgende block macht es moeglich beliebig viele defaultwert-dateien
  // anzugeben als massnahme gegen schleifen der inklusion gibt es den
  // defaultvalues-zaehler bei jedem weiteren auslesen der defaultwerte der
  // 'naechst-tiefen' datei wird der zaehler um eins hoch gezaehlt
  // syntax:
  //   //Block DefaultValues
  ...
  if (blockname != "DefaultValues") {
    ...
    if (10 < _i_defaultvalues_level) {
      cerr << __FILE__ << ":" << __LINE__
           << ": there seems to be a 'Block DefaultValues' loop" << endl;
      cerr << __FILE__ << ":" << __LINE__
           << ": last used file: " << pf.GetName() << endl;
      abort();
    }
  ...
  if (_i_defaultvalues_level == 1 && _i_defaultvalues_save_all_to_file) {
    ...
    //
    // one can save *all* paramfile values that are used by Gascoigne. This
    // includes also the default values of options that the user did not
    // explicitly set in the paramfile. (It also includes values that may have
    // been loaded in other include files. The DefaultValues Block is not saved,
    // since the include commands don't belong in a 'flattened file') This is a
    // good and easy way of determining and listing all availible Gascoigne
    // options.
    //
    ...
    if (!fcheckforfile.is_open()) {
      ...
      // the perl script reads "itself" and generates the complete paramfile
      // automatically (it writes to stdout); the values and blocks may occur
      // multiply in the file, only the last entries are used
      ofstream createheader(_s_defaultvalues_save_filename.c_str(),
                            ios_base::out);
    ...
    // this way the file can be used to achive the history of changes in a
    // paramfile; the perl script only uses the last changes in the file; the
    // date tag is ignored by the script
    char ca_date[100];
    {
      time_t t_date;
      struct tm* tmzgr;
    ...
    savefile << "<Block name='" << blockname << "' file='" << pf.GetName()
             << "' date='" << ca_date << "'>\n";
    DH.print(savefile);
    savefile << "</Block>\n\n";
    savefile.close();
...
FileScanner::FormatToValue(const vector<string>& words)
{
  ...
  /*----------------------------------------------*/
    ...
    cerr << " ********  FileScanner:: keyword '" << keyword
         << "' found in param-file but not registered in the "
            "DataFormatHandler-code."
         << endl;
  }
}
