...
/**
 * Wandelt die lokale Nummerierung der Zellen eines Patches von der
 * Gascoigne-nummerierung in die vom Integrator verwendete um! Funktioniert aus
 * Symmetrie-Gruenden auch andersherum. Erwartet eine Zahl zwischen 0 und 3
 * (2-D) bzw 0 bis 7 (3-D)
 */
...
IntegratorQ1Q2<DIM>::PatchMeshNr2IntegratorNr(int in) const
{
  ...
  std::cerr << "Integrator Q1/Q2: Dimension " << DIM << " nicht implementiert!"
            << std::endl;
  abort();
}
                          ...
                          const LocalData& QC) const
{
  ...
  if (DIM == 2)
    ...
    IF = new PatchFormula3d<27, HexGauss27>;
  ...
  if (IF->n() % numcells != 0) {
    std::cerr << "Integrator Q1/Q2: Falsche Anzahl Zellen je Patch oder "
