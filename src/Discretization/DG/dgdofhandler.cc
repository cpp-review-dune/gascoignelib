...
namespace Gascoigne
{
	...
	for (int e=0;e<4;++e)
	  {
		...
		E[3] = e; // local index
	      ...
	      {
		std::cerr << "Construction of Edge failed "
			  << E[0] << " " << E[1] << " " << E[2] << " " << E[3] 
			  << "\t" << ioc << std::endl;
		abort();
	      }
  ...
  template <class BASE>
  void DGDofHandler<BASE>::GlobalToLocal(LocalVector &U,
  ...
  template <class BASE>
  void DGDofHandler<BASE>::LocalToGlobal(GlobalVector &f,
                                         const LocalVector &F,
                                         int iq,
                                         double s) const
  {
  ...
  template <class BASE>
  void DGDofHandler<BASE>::LocalToGlobalMatrix(MatrixInterface& A, EntryMatrix& E, int iq, double s) const
  {
    assert(iq < nelements());
    const ElementType &LI = getelement(iq);
  ...
  template <class BASE>
  void DGDofHandler<BASE>::LocalToGlobalMatrix(MatrixInterface& A, EntryMatrix& E, int q1, int q2 , double s) const
  {
    assert(q1 < nelements());
    assert(q2 < nelements());
