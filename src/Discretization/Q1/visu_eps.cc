...
VisuEPS::WriteGrid(string fname, int iter)
{
  ...
  out << "%!PS-Adobe-2.0 EPSF-1.2" << endl
      ...
      << static_cast<unsigned int>((y_max - y_min) * scale) + 10 << endl;

  // define some abbreviations to keep
  // the output small:
  // m=move turtle to
  // x=execute lineto
  out << "/m {moveto} bind def" << endl << "/x {lineto} bind def" << endl;
...
VisuEPS::CombineLines()
{
  ...
  do {
    ...
    changed = false;
    for (int i = 0; i < lines.size(); ++i) {
      for (set<int>::iterator it = lines[i].begin(); it != lines[i].end();
           ++it) {
        int next = *it;
        bool found = false;
        set<int>::iterator it1;
        int nn;
        for (it1 = lines[next].begin(); ((it1 != lines[next].end()) & (!found));
             ++it1) {
          nn = *it1;
  ...
  //   cerr << "Combined: " << combine << " of " << n_lines << " lines in " <<
  //   steps << " steps.\n";
}
} // namespace Gascoigne
