...
namespace Gascoigne {
template <int DIM> class EnergyEstimatorIntegrator : public BasicIntegrator {
protected:
...
public:
  ...
  void Jumps(LocalVector &F, const FemInterface &FEM, const LocalVector &U,
             int ile) const;
  double JumpNorm(const FemInterface &FEM,
                  std::array<double, 2 * DIM - 2> jumps, int ile) const;
  double Residual(const LocalVector &U, const FemInterface &FEM,
