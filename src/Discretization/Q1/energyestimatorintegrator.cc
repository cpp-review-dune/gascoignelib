                                      ...
                                      int ile) const
{
  ...
  for (int i = 0; i < 2 * DIM - 2; i++) {
    ...
    if (_s_energytype == "energy" || _s_energytype == "energy_laplace") {
    ...
    } else if (_s_energytype == "energy_stokes") {
      ...
      std::cerr << " Bad EnergyEstimatorIntegrator type supplied: '"
                << _s_energytype << "'.\n";
      std::cerr << " Please specify a known EnergyEstimatorIntegrator type: "
                   "enery_laplace or enery_stokes.\n";
      abort();
    }
  }
}
