...
namespace Gascoigne
{
  ...
  void CellDiscretization::BoundaryForm(GlobalVector& f, const GlobalVector& u, const ProblemDescriptorInterface& PD, double d) const
  {
  ...
  void CellDiscretization::BoundaryMatrix(MatrixInterface& A, const GlobalVector& u, const ProblemDescriptorInterface& PD, double d) const
  {
    if (!PD.NewBoundaryEquation())
      return;
    const auto *BE = PD.NewBoundaryEquation();
