...
public:
  ...
  const Matrix& DT() const
  {
    return dt;
  }
  const Matrix& DTI() const
  {
    return dti;
  }
...
template <class BASE>
inline const nvector<nmatrix<double>> Transformation3d<BASE>::DDTI(
...
/*-----------------------------------------------------*/
template <class BASE>
inline const nvector<nmatrix<double>> Transformation3d<BASE>::ComputeDDT(
  const Vertex3d& xi) const
{
