...
public:
  ...
  SparseBlockMatrix<B>(bool cuda = false);
  SparseBlockMatrix<B>(const SparseBlockMatrix<B>& A, bool cuda = false);
  ...
  {
    return US.start(i + 1) - US.start(i);
  }
  ...
  void AddMassWithDifferentStencil(const MatrixInterface* M, const TimePattern& TP,
                                   double s = 1.);
  void AddMassWithDifferentStencilJacobi(const MatrixInterface* M, const TimePattern& TP,
                                         double s = 1.);
  ...
  void entry(nvector<int>::const_iterator start, nvector<int>::const_iterator stop,
             const EntryMatrix& M, double s = 1.);
  void entrydual(nvector<int>::const_iterator start, nvector<int>::const_iterator stop,
                 const EntryMatrix& M, double s = 1.);
  ...
  void FurbishInterface(double d, const nvector<int>& elements, const nvector<int>& start,
  ...
  {
    std::cerr
      << "\"ostream& operator<<(ostream &s, const SparseBlockMatrix<B>& A)\" not written!"
      << std::endl;
    abort();
  }
};
