...
public:
  ...
  template <class IT> void build_add(int i, IT lsf, IT lsl) {
    for (IT p = lsf; p != lsl; p++)
      row(i).insert(*p);
  }
  template <class IT> void build_add(IT lsf, IT lsl) {
    for (IT p = lsf; p != lsl; p++)
      build_add(*p, lsf, lsl);
  }
  template <class IT> void build_add(IT rf, IT rl, IT cf, IT cl) {
    for (IT p = rf; p != rl; p++)
      build_add(*p, cf, cl);
  }
  void build_end();
