...
#pragma omp parallel for
  ...
  void VankaSmoother::copy_entries(const MatrixInterface& A)
  {
    ...
    else if (_ncomp == 5) copy_entries_sparseblockmatrix(dynamic_cast<const SparseBlockMatrix<FMatrixBlock<5> >&>(A));
    else if (_ncomp == 6) copy_entries_sparseblockmatrix(dynamic_cast<const SparseBlockMatrix<FMatrixBlock<6> >&>(A));
    else if (_ncomp == 7) copy_entries_sparseblockmatrix(dynamic_cast<const SparseBlockMatrix<FMatrixBlock<7> >&>(A));   
  }
