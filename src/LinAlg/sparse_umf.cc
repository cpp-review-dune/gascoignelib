...
/*-----------------------------------------*/
  ...
  template<class B>
  double SparseUmf<B>::exec(std::string cmd) const
  {
  ...
  template<class B>
  void SparseUmf<B>::do_precondition()
  {
    abort();
  }

  template<class B>
  void SparseUmf<B>::undo_precondition()
  {
    abort();
  }
  ...
  template<class B>
  double SparseUmf<B>::condition_number()  const
  {
    int N = __Ap.size()-1;
    assert(N>0);
    ...
    ss.clear();
    ss.str("");
    ss << "/nfs/software/matlab/bin/matlab  -nojvm -nodesktop -nodisplay -nosplash < " << mname
