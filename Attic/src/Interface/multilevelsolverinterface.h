...
public:
  ...
  virtual std::string LinearSolve(int level, VectorInterface &u,
                                  const VectorInterface &b, CGInfo &info) = 0;
  virtual std::string LinearSolve(VectorInterface &u, const VectorInterface &b,
                                  CGInfo &info) {
    return LinearSolve(nlevels() - 1, u, b, info);
  }

  virtual std::string Solve(int level, VectorInterface &x,
                            const VectorInterface &b, NLInfo &nlinfo) = 0;
  ...
  virtual void newton(VectorInterface &u, const VectorInterface &f,
                      VectorInterface &r, VectorInterface &w, NLInfo &info) = 0;

  virtual std::vector<MgInterpolatorInterface *> &GetInterpolatorPointers() = 0;
  virtual const std::vector<MgInterpolatorInterface *> &
  GetInterpolatorPointers() const = 0;
};
} // namespace Gascoigne
