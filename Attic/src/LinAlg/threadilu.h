...
public:
  ...
  virtual void
  ConstructStructure(const std::vector<IntVector> &perm,
                     const MatrixInterface &A,
                     const std::vector<std::vector<int>> &domains2nodes);

  virtual void ConstructStructure(const Gascoigne::IntVector &,
                                  const Gascoigne::MatrixInterface &) {
    std::cerr
        << "\"ThreadIlu::ConstructStructure(const Gascoigne::IntVector&, const "
           "Gascoigne::MatrixInterface&)\" has no meaning, please use !"
        << std::endl;
    std::cerr
        << "\"ThreadIlu::ConstructStructure(const  "
           "std::vector<std::vector<int> >& perm, const MatrixInterface& A, "
           "const std::vector<std::vector<int> > domains2nodes)\" instead!"
        << std::endl;
    abort();
  }
