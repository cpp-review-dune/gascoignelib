...
template <int DIM> LpsIntegratorQ2<DIM>::~LpsIntegratorQ2() {
...
template <int DIM> LpsIntegratorQ1<DIM>::~LpsIntegratorQ1() {
  if (LpsIntegrator<DIM>::_IF)
    delete LpsIntegrator<DIM>::_IF;
  LpsIntegrator<DIM>::_IF = 0;
}
...
template <int DIM>
void LpsIntegratorQ1<DIM>::VectorReinit(LocalVector &F, int n,
                                        int ncomp) const {
  F.ReInit(ncomp, n);
  F.zero();
}
...
template <int DIM>
void LpsIntegratorQ1<DIM>::Form(const Equation &EQ, LocalVector &F,
...
template <int DIM>
void LpsIntegratorQ1<DIM>::Matrix(const Equation &EQ, EntryMatrix &E,
                                  const FemInterface &FEM, const LocalVector &U,
                                  const LocalData &Q,
