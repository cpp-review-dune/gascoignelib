...
template<int DIM>
void
LpsIntegratorQ1<DIM>::VectorReinit(LocalVector& F, int n, int ncomp) const
{
  F.ReInit(ncomp, n);
  F.zero();
}
...
template<int DIM>
void
LpsIntegratorQ1<DIM>::Form(const Equation& EQ,
                           LocalVector& F,
...
template<int DIM>
void
LpsIntegratorQ1<DIM>::Matrix(const Equation& EQ,
                             EntryMatrix& E,
                             const FemInterface& FEM,
                             const LocalVector& U,
                             const LocalData& Q,
...
template<int DIM>
void
LpsIntegratorQ1<DIM>::ResetMatrix(EntryMatrix& E, int n, int ncomp) const
{
