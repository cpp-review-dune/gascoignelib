...
template <> void FaceQ1<2>::build_faces() {
  ...
  for (std::map<pair<int, int>, std::array<int, 2>>::const_iterator it =
    ...
    int n0 = -1, n1 = -1, dir = 1;
    for (n0 = 0; n0 < 4; ++n0)
      if (GM->vertex_of_cell(c0, n0) == it->first.first)
        break;
    assert(n0 < 4);
    for (n1 = 0; n1 < 4; ++n1)
      if (GM->vertex_of_cell(c0, n1) == it->first.second)
        break;
    assert(n1 < 4);
    if ((n0 + dir) % 4 != n1)
      dir = -1;
    ...
    dir = 1;
    for (n0 = 0; n0 < 4; ++n0)
      if (GM->vertex_of_cell(c1, n0) == it->first.first)
        break;
    assert(n0 < 4);
    for (n1 = 0; n1 < 4; ++n1)
      if (GM->vertex_of_cell(c1, n1) == it->first.second)
        break;
    assert(n1 < 4);
    if ((n0 + dir) % 4 != n1)
      dir = -1;
