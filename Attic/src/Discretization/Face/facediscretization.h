...
protected:
  ...
  virtual void TransformationFace(FemInterface::Matrix &T1,
                                  FemInterface::Matrix &T2, int f) const {
    std::cerr << "FaceDiscretization::Transformation not written!" << std::endl;
    abort();
  }
  virtual void LocalToGlobalFace(GlobalVector &f, const LocalVector &F, int iq,
                                 double s) const;
  virtual void GlobalToLocalFace(LocalVector &U, const GlobalVector &u,
                                 int iq) const;
  virtual void LocalToGlobalFace(MatrixInterface &A, EntryMatrix &E, int iq,
                                 double s) const;
...
public:
  ...
  virtual void BasicInit(const ParamFile *pf) {
    std::cerr << "\"FaceDiscretization::BasicInit not written\"" << std::endl;
    abort();
  }
  virtual void ReInit(const GascoigneMesh *M) {
    std::cerr << "\"FaceDiscretization::ReInit not written\"" << std::endl;
    abort();
  }
  virtual void build_faces() {
    std::cerr << "\"FaceDiscretization::build_faces not written\"" << std::endl;
    abort();
  }
  ...
  virtual const FemInterface *GetFem1() const {
    assert(__FEM1);
    return __FEM1;
  }
  virtual const FemInterface *GetFem2() const {
    assert(__FEM2);
    return __FEM2;
  }
  ...
  virtual const FaceIntegratorInterface *GetFaceIntegrator() const {
    assert(__INT);
    return __INT;
  }
  ...
  virtual void FaceForm(GlobalVector &f, const GlobalVector &u,
                        const FaceEquation &EQ, double d) const;
  virtual void FaceMatrix(MatrixInterface &A, const GlobalVector &u,
                          const FaceEquation &EQ, double) const;
};
} // namespace Gascoigne
