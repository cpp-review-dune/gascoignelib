    ...
    if (DIM == 2) {
      ...
      for (int iy = 0; iy < Q + 1; ++iy)
        for (int ix = 0; ix < 2 * Q + 1; ++ix) {
          ...
          _N2.zero();
          if (ix < Q + 1)
            FEM1.init_test_functions(_N1, weight, iy * (Q + 1) + ix);
          if (ix > Q - 1)
            FEM2.init_test_functions(_N2, weight, iy * (Q + 1) + ix - Q);
    ...
    if (DIM == 2) {
      ...
      for (int iy = 0; iy < (Q + 1); ++iy)
        for (int ix = 0; ix < 2 * Q + 1; ++ix) {
          ...
          NNN2[i].zero();
          if (ix < Q + 1)
            FEM1.init_test_functions(NNN1[i], sw, iy * (Q + 1) + ix);
          if (ix > Q - 1)
            FEM2.init_test_functions(NNN2[i], sw, iy * (Q + 1) + ix - Q);
        }
    } else if (DIM == 3) {
