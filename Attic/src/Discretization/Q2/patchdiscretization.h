...
protected:
  ...
  virtual int GetPatchNumber(const Vertex2d &p0, Vertex2d &p) const {
    std::cerr << "\"PatchDiscretization::GetPatchNumber\" not written!"
              << std::endl;
    abort();
  }
...
public:
  ...
  void Form(GlobalVector &f, const GlobalVector &u,
            const ProblemDescriptorInterface &EQ, double d) const;
  ...
  virtual nmatrix<double> GetLocalInterpolationWeights(int iq) const {
    std::cerr
        << "\"PatchDiscretization::GetLocalInterpolationWeights\" not written!"
        << std::endl;
    abort();
  }
};
