/**
*
* Copyright (C) 2004, 2005, 2006, 2007, 2009, 2010, 2011 by the Gascoigne 3D
*authors
*
...
namespace Gascoigne
{
  ...
  void Q12d::StrongPeriodicVector(GlobalVector &u,
  ...
  void Q12d::EnergyEstimator(EdgeInfoContainerInterface &EIC,
                             DoubleVector &eta,
  ...
  void Q12d::EEJumps(EdgeInfoContainer<2> &EIC,
                     const GlobalVector &u,
  ...
  void Q12d::EEJumpNorm(EdgeInfoContainer<2> &EIC,
                        DoubleVector &eta,
                        const EnergyEstimatorIntegrator<2> &EEI,
                        const HierarchicalMesh2d *HM) const
  {
  ...
  void Q12d::EEResidual(DoubleVector &eta,
                        const GlobalVector &u,
                        const Equation &EQ,
                        const DomainRightHandSide *RHS,
  ...
  int Q12d::GetCellNumber(const Vertex2d &p0, Vertex2d &p, int c0) const
  {
    if (c0 != 0)
    {
      ...
      if ((p[0] > 0. - 1.e-12) && (p[0] < 1. + 1.e-12) &&
          (p[1] > 0. - 1.e-12) && (p[1] < 1. + 1.e-12))
      {
        return c0;
      }
    }
  ...
  void Q12d::VertexTransformation(const Vertex2d &p0,
                                  Vertex2d &p,
                                  int iq,
                                  bool abortiffail) const
  {
    ...
    for (int niter = 1;; niter++)
    {
      ...
      if (niter >= 10)
      {
        cerr << "void Q12d::VertexTransformation(const Vertex2d& p0, Vertex2d& "
                "p, int iq) const"
             << endl
             << "\t did not converge " << res << "\t" << p << "\t" << p0
             << endl;
        if (abortiffail)
          abort();
        else
  ...
  void Q12d::RhsCurve(GlobalVector &F, const Curve &C, int comp, int N) const
  {
    ...
    int c0 = GetCellNumber(x0, xr0),
        c1; // Nummern derjenigen Zellen, in denen x0, x1 liegen
    // GetCellNumber -> bestimmt die Zelle, in der Punkt x0 liegt, und den
    // korrespondierenden Punkt in der Referenzzelle xr0.
