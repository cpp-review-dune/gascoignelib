...
namespace Gascoigne {
template <class BASE> class DG : public DiscretizationInterface {
private:
...
public:
  ...
  void EdgeForm(GlobalVector &f, const GlobalVector &u, const DGEquation &EQ,
                double d) const;
  void EdgeRhs(GlobalVector &f, const DomainRightHandSide &RHS, double s) const;
  void EdgeMatrix(MatrixInterface &A, const GlobalVector &u,
                  const DGEquation &EQ, double) const;
  ...
  void StabForm(GlobalVector &f, const GlobalVector &u, const Equation &EQ,
                double d) const {
  ...
  void StrongPeriodicVector(GlobalVector &u, const PeriodicData &BF, int col,
                            const std::vector<int> &comp, double d = 1.) const {
  }
};
} // namespace Gascoigne
