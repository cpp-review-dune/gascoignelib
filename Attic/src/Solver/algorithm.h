...
protected:
  ...
  const NumericInterface *GetNumeric() {
    assert(_NI);
    return _NI;
  }
  SolverInfos *&GetSolverInfosPointer() { return _SI; }
  SolverInfos *GetSolverInfos() {
    assert(_SI);
    return _SI;
  }
  const SolverInfos *GetSolverInfos() const {
    assert(_SI);
    return _SI;
  }
  ...
  virtual void LinearSolve(VectorInterface &du, const VectorInterface &y,
                           CGInfo &cginfo) {
...
public:
  ...
  const MeshAgentInterface *GetMeshAgent() const {
    assert(_MA);
    return _MA;
  }
  MeshAgentInterface *GetMeshAgent() {
    assert(_MA);
    return _MA;
  }
  ...
  virtual void BasicInit(const ParamFile *paramfile, const NumericInterface *NI,
                         const ProblemContainer *PC = NULL);
};
} // namespace Gascoigne
