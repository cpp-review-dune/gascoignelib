...
void NewmarkSolver::Form(VectorInterface &gy, const VectorInterface &gx,
                         double d) const {
...
void NewmarkSolver::FormWithoutMass(VectorInterface &gy,
                                    const VectorInterface &gx, double d,
                                    double s) const {
  HNAverage(gx);
  HNAverageData();
  ...
  if (BE) {
    ...
    GetDiscretization()->BoundaryForm(GetGV(gy), GetGV(gx),
                                      BM->GetBoundaryEquationColors(), *BE,
                                      d * sqrt(alpha) * s);
  }
  HNZero(gx);
  HNZeroData();
...
void NewmarkSolver::AssembleMatrix(const VectorInterface &gu, double d) {
  ...
  GetDiscretization()->Matrix(
      *GetMatrix(), u, *GetProblemDescriptor()->GetEquation(), d * alpha);
  const BoundaryEquation *BE = GetProblemDescriptor()->GetBoundaryEquation();
  if (BE) {
    const BoundaryManager *BM = GetProblemDescriptor()->GetBoundaryManager();
    GetDiscretization()->BoundaryMatrix(
        *GetMatrix(), u, BM->GetBoundaryEquationColors(), *BE, d * sqrt(alpha));
  }
  DirichletMatrix();
  HNZero(gu);
  HNZeroData();
