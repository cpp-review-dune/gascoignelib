  ...
  IndexType cols)
{
  ...
  reorganise(matrixdata, CS, csrValsDevice, csrRowIdxsDevice, csrColIdxsDevice);
...
template<int B>
void
CudaCSRMatrixInterface::copy_data(const SparseBlockMatrix<FMatrixBlock<B>>& sbm)
{
  _n_comp = B;
...
CudaCSRMatrixInterface::get_inverse_diagonal(
  ...
  const SparseBlockMatrix<FMatrixBlock<B>>& sbm)
{
  ...
  auto device_mat =
    std::make_shared<CudaCSRMatrixInterface>(sparse_handle, n, n, n_comp);
 ...
 */
std::shared_ptr<CudaCSRMatrixInterface>
CudaCSRMatrixInterface::get_inverse_diagonal(cusparseHandle_t sparse_handle,
                                             const MatrixInterface& mat)
{
  try {
...
// template<int B>
// void
// CudaCSRMatrixInterface::operator=(const std::vector<FMatrixBlock<B>>&
// matrixdata)
// {
//   size_t n_values = _n_entries * _n_comp * _n_comp;
